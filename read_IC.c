...
int main(int argc, char **argv)
{
  ...
  sprintf(path, "/nobackupp1/astacy/ICs");
  sprintf(basename, "midres_seed");
  ...
  outfile=fopen("/nobackupp1/astacy/ICs//snapminihalo_256","w"); 
  ncount = 0;
  ncount2 = 0;
  for(n=1;n<=Ngas;n++) { 
    x=P[n].Pos[0];
    ...
    /*  printf("RAND_MAX = %12d \n",RAND_MAX);
    printf("random= %12d \n",random);  */
    if((random*(1.e0/RAND_MAX))< 0.1e0 ){
      fprintf(outfile,"%8d %8d %15.6g %15.6g %15.6g %15.6g %15.6g %15.g\n", 
	      n, Id[n],x,y,z,P[n].Vel[2],P[n].Mass, P[n].U);
      ncount = ncount + 1;
    }
...
printf("ntot_withmasses = %d\n",ntot_withmasses);
	...
	SKIP;
     ...
     for(k=0;k<10;k++)
      {
       printf("header1.mass = %lg\n",header1.mass[k]);
       printf("header1.npart = %d\n",header1.npart[k]);
      } 
