...
int main(int argc, char **argv)
{
  ...
  double vel_arr[arrnum], velx_arr[arrnum], vely_arr[arrnum], velz_arr[arrnum], vradx_arr[arrnum], vrady_arr[arrnum], vradz_arr[arrnum], vrotx_arr[arrnum], vroty_arr[arrnum], vrotz_arr[arrnum];
  double rmin, rmax, nmin, nmax, hubble;
  FILE *outfile;
  ...
  sprintf(output_fname, "%s_turb_%03d.dat",basename, snapshot_number);
  ...
  nthresh = 2.e11;
   ...
   for(n=0; n<arrnum; n++)
     {
     m_enc_arr[n] = num_arr[n] = turb_arr[n] = c_s_arr[n] = vrad_arr[n] = vrot_arr[n] = 0.0;
  ...
  for(n=0;n<Ngas;n++) {
             ...
             //if(disAU <= 1.e1 /*&& P[n].sink < -4*/)
              ...
              for(k=0; k<=0; k++)
                  ...
                  if(disAU < d_arr[0])
                     ...
                     c_s_arr[k] = c_s_arr[k] + pow(1.38e-16*P[n].Temp/1.e-24,0.5)/1.e5;
   ...
   for(n=0; n<=arrnum; n++)
     ...
     //printf("velx = %lg, vely = %lg, velz = %lg\n", velx_arr[n], vely_arr[n], velz_arr[n]);
     //printf("vradx = %lg, vrady = %lg, vradz = %lg\n", vradx_arr[n], vrady_arr[n], vradz_arr[n]);

      //turb_arr[n] = turb_arr[n]/num_arr[n];
      fprintf(outfile, "%15.11g %15.11g %15.11g %15.11g %15.11g %15.11g\n",  d_arr[n], c_s_arr[n], vrad_arr[n], vrot_arr[n], turb_arr[n], m_enc_arr[n]);
      }
   fclose(outfile);
