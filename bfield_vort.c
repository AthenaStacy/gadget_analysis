...
//    int n_grid2[N_grid][N_grid];
    ...
    double max = 1.e-18;
    if(do_error == 1) {min = 1.e-3; max = 1.e0;}    
    double abs_min = min;
    double abs_max = max;
    ...
    sprintf(dir, "/scratch/00863/minerva/vort");
    sprintf(dir_out, "/scratch/00863/minerva/vort");
    ...
    if(which_sim == 2) 
      {
      sprintf(dir, "/scratch/00863/minerva/vort_corr");
      sprintf(dir_out, "/scratch/00863/minerva/vort_corr");
      }
    if(which_sim == 3) 
      {
      sprintf(dir, "/scratch/00863/minerva/vort_divsmooth");
      sprintf(dir_out, "/scratch/00863/minerva/vort_divsmooth");
      }
    if(which_sim == 4) 
      {
      sprintf(dir, "/scratch/00863/minerva/vort_jiggle");
      sprintf(dir_out, "/scratch/00863/minerva/vort_jiggle");
      }
    if(which_sim == 5)
      {
      sprintf(dir, "/scratch/00863/minerva/vort");
    ...
    if(which_sim == 6)
      {
      sprintf(dir, "/scratch/00863/minerva/vortR");
      sprintf(dir_out, "/scratch/00863/minerva/vortR");
      }
   ...
   if(do_error == 1) sprintf(endtype, ".error");
   if(anal_switch == 1) sprintf(endtype, ".banal");
...
//Read in B-field info!///////////////////////////////////////////////////////////
  ...
  if(which_sim == 5)
    {
    sprintf(path2, "/work/00863/minerva/orion/vort_hires");
    sprintf(basename2, "vort");
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////// 
        ...
        for(n=0;n<Ngas;n++)
           ...
           if(P[n].Bmag > Bmax) Bmax = P[n].Bmag;
           }

        Bmean = Btot / ntot;
        Brms = pow(Bmean, 0.5);
        printf("Brms = %lg Bmax = %lg\n", Brms, Bmax);
...
if(x2 <= 1.0)
  {
    ...
    if(do_error == 1)  grid1[i][j] += weight * fabs(P[n].error) * W_x;
    else if(comp == 1) grid1[i][j] += weight * fabs(P[n].Bfieldx) * W_x;
