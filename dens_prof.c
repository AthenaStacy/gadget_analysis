...
int main(int argc, char **argv)
{
  ...
  int  i, k=0, j, n, type, snapshot_number, files, Ngas, random, ncount, ncounthalo1, arrnum=100;
  ...
  if(halo == 8)
     ...
     //jnum = 10;
     //jnum = 150;  //1000 yr ? 2500 is renumbered to ~230 (234)
     jnum = 400; //5000 yr
     }
 if(halo == 10)
     {
  ...
  if(halo == 12)
     {
     sprintf(basename, "midhires_testLW3");
     sprintf(basenameout, " ");
     jnum = 2;
     }
  ...
  sprintf(output_fname2, "%s_dens_%03d.dat",basename, snapshot_number);
		   ...
		   ncount++;  
                 ...
                 if(P[i].nh > 1.e1 && P[i].sink > -4)
                   for(n=0; n<arrnum; n++) 
                    ...
                    if(disAUxy > d_arr[n] && disAUxy < d_arr[n+1] && P[i].nh > 0.5*dens_arr[n]/n_arr[n] && P[i].sink < 0.5)
                    //if(disAUxy < d_arr[n] && disAUz < fmax(300,0.3*d_arr[n]) && P[i].sink < 0.5)
                    //if(disAUxyz > d_arr[n] && disAUxyz < d_arr[n+1] && P[i].sink < 0.5)
                    //if(disAU < d_arr[n+1] && P[i].nh > 2.0*dens_arr[n]/n_arr[n] && P[i].sink < 0.5)
                     {
             ...
             for(n=0; n<arrnum; n++)
                   ...
                   vrot_arr[n] = pow(vradx_arr[n]*vradx_arr[n] + vrady_arr[n]*vrady_arr[n] + vradz_arr[n]*vradz_arr[n], 0.5)/n_arr[n];
                   ...
                   fprintf(outfile2, "%15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g\n", d_arr[n], m_enc_arr[n], dens_arr[n], surf_dens_arr[n], c_s_arr[n], temp_arr[n], mbe_arr[n], omega_arr[n]/m_enc_arr[n], vrad_arr[n], vrot_arr[n]);
                   }
             fclose(outfile);
             fclose(outfile2);
