...
#define width 10.0
#define width_small 10.e0  //full width in pc of cubic box of particles whose evolution will be tracked
...
int main(int argc, char **argv)
{
  ...
  double hfac = 2.0, hfac_in = 0.0, hfac_avg, time_fac, time_fac0, cosmo_fac, cosmo_fac0;
             ...
             if(write_file == 1)
                { 
                 if(P[i].marker > 0)
                   ...
                   PBfieldx[i] = B0x * pow(P[i].Rho_phys/rho_baryon, 2./3.) / (double) tot_proc_sum;
                   PBfieldy[i] = B0y * pow(P[i].Rho_phys/rho_baryon, 2./3.) / (double) tot_proc_sum;
                   PBfieldz[i] = B0z * pow(P[i].Rho_phys/rho_baryon, 2./3.) / (double) tot_proc_sum;
                   }
                 }
               ...
               for(n=nmin; n<=nmax; n++)
                      ...
                      if(P[i].Id == plist_id[n])
                         ...
                         if(j == 0)
                           ...
                           plist1[n].B_anal[0] = B0x * pow(P[i].Rho_phys/rho_baryon, 2./3.);
                           plist1[n].B_anal[1] = B0y * pow(P[i].Rho_phys/rho_baryon, 2./3.);
                           plist1[n].B_anal[2] = B0z * pow(P[i].Rho_phys/rho_baryon, 2./3.);
                           }
                          if(j == 0 && restart == 1)
                           {
        ...
        if(j > 0)
         {
          ...
          double randomx, randomy, randomz, hfac2=0.5, hfac3 = 0.1;
...
/*
                     ...
                     nb[n-nmin].pos0[0][k] = plist0[n].pos[0] + hfac2 * plist0[n].hsm - randomx * 2 * hfac2 * plist0[n].hsm;
                     nb[n-nmin].pos0[1][k] = plist0[n].pos[1] - hfac2 * plist0[n].hsm + randomy * 2 * hfac2 * plist0[n].hsm;
                     nb[n-nmin].pos0[2][k] = plist0[n].pos[2] + hfac2 * plist0[n].hsm - randomz * 2 * hfac2 * plist0[n].hsm;
*/
...
///////////////////////////////////////////////////////////
            ...
            for(n=nmin; n<=nmax; n++)
                ...
                for(p=0; p<3; p++)
                 ...
                 int neighb_min = 5;                 
                 if(which_sim == 2 || which_sim == 3) neighb_min = 30;
                 ...
                 if(plist0[n].nh_anal <= 0 || plist0[n].nh_anal != plist0[n].nh_anal)
                   ...
                   plist0[n].B_anal[0] = B0x * pow(plist0[n].dens/rho_baryon, 2./3.);
                   plist0[n].B_anal[1] = B0y * pow(plist0[n].dens/rho_baryon, 2./3.);
                   plist0[n].B_anal[2] = B0z * pow(plist0[n].dens/rho_baryon, 2./3.);
                   printf("FIXING IT3, n = %d, dens = %lg, plist0[n].B_anal[2] = %lg\n", n, plist1[n].dens, plist0[n].B_anal[2]);
                   }
                ...
                for(p=0; p<3; p++)
                 ...
                 if(n < 5) 
                 printf("j = %d, n = %d, n_neighb = %d, dens0 = %lg, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, B0 = %lg, hsm0 = %lg, hsm1 = %lg, deform = %lg, detD = %lg, detD_anal = %lg, ERROR = %lg, B_anal[0] = %lg, B_anal[1] = %lg, B_anal[2] = %lg\n",
                    j, n, plist1[n].NumNeighb, plist0[n].dens, plist0[n].nh_anal, plist1[n].dens,  plist1[n].nh_anal, plist0[n].B_anal[2], plist0[n].hsm, plist1[n].hsm, deform_avg[0][0],
