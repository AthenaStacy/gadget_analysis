...
int main(int argc, char **argv)
{
  ...
  double xfac, yfac, zfac, kfloat, nh_avg[snapnum], nh_avg2, num_tot, omega, omega0, nthresh=1.e0;
  ...
  double davgHI0[3], davgLO0[3], davgHI1[3], davgLO1[3], davgHI2[3], davgLO2[3], numHI0[3], numLO0[3], numHI1[3], numLO1[3], numHI2[3], numLO2[3];
  ...
  sprintf(basename, "bin_zoom10_ref");
          ...
          for(i = 0; i < Ngas; i++)
             ...
             for(n=0; n<arrnum; n++)
               for(k=0; k<plist1[n].NumNeighb-1; k++)
                  ...
                  if(P[i].Id == nb[n].id[k])
                     {
                     nb[n].pos2[0][k] = P[i].Pos[0];
                     nb[n].pos2[1][k] = P[i].Pos[1];
                     nb[n].pos2[2][k] = P[i].Pos[2];
                     nb[n].vel2[0][k] = P[i].Vel[0];
                     nb[n].vel2[1][k] = P[i].Vel[1];
                     nb[n].vel2[2][k] = P[i].Vel[2];
                     nb[n].nh2[k] = P[i].Rho;
                     }
                    }
              }
            ...
            for(n=0; n<arrnum; n++)
                ...
                for(p=0; p<3; p++)
                  davgHI0[p]=davgLO0[p]=davgHI1[p]=davgLO1[p]=davgHI2[p]=davgLO2[p]=numHI0[p]=numLO0[p]=numHI1[p]=numLO1[p]=numHI2[p]=numLO2[p]=0;
                ...
                for(k=0; k<plist1[n].NumNeighb-1; k++)
                  ...
                  dis2[k] = pow(((nb[n].pos2[0][k]-plist2[n].pos[0])*(nb[n].pos2[0][k]-plist2[n].pos[0])
                       + (nb[n].pos2[1][k]-plist2[n].pos[1])*(nb[n].pos2[1][k]-plist2[n].pos[1])
                       + (nb[n].pos2[2][k]-plist2[n].pos[2])*(nb[n].pos2[2][k]-plist2[n].pos[2])), 0.5);
...
*/
                     ...
                     for(p=0; p<3; p++)
                      { 
                       if(fabs(plist0[n].pos[p] - nb[n].pos0[p][k]) < hfac_avg*plist0[n].hsm)  
                        ...
                        if(fabs(plist2[n].pos[p] - nb[n].pos2[p][k]) < hfac_avg*plist2[n].hsm)
                         {
                         davgLO2[p] = davgLO2[p] + fabs(plist2[n].pos[p] - nb[n].pos2[p][k]);
                       ...
                       if(fabs(plist0[n].pos[p] - nb[n].pos0[p][k]) > hfac_avg*plist0[n].hsm)
                        ...
                        if(fabs(plist2[n].pos[p] - nb[n].pos2[p][k]) > hfac_avg*plist2[n].hsm)
                         {
                         davgHI2[p] = davgHI2[p] + fabs(plist2[n].pos[p] - nb[n].pos2[p][k]);
                ...
                for(p=0; p<3; p++)
                  {
                  printf("davgLO0= %lg, davgLO1 = %lg, davgLO2 = %lg, davgHI0=%lg, davgHI1 = %lg, davgHI2 = %lg, numLO0 = %lg, numLO1 = %lg, numLO2 = %lg, numHI0 = %lg, numHI1 = %lg, numHI2 = %lg\n",
                          davgLO0[p], davgLO1[p], davgLO2[p], davgHI0[p], davgHI1[p], davgHI2[p], numLO0[p], numLO1[p], numLO2[p], numHI0[p], numHI1[p], numHI2[p]);
                ...
                for(p=0; p<3; p++)
                 ...
                 plist1[n].detD = plist1[n].detD/plist1[n].NumCalc;
                ...
                for(p=0; p<3; p++)
                   for(q=0; q<3; q++)
                     deform_avg[p][q] =  (-(davgHI1[p] - davgLO1[p]) + (davgHI2[p] - davgLO2[p]))/(davgHI1[q] - davgLO1[q]);
