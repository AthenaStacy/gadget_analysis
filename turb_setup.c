...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  double ngrid_left_tot = 0, ngrid_tot = 0;
  double frac_xleft=0, frac_yleft=0, frac_zleft=0, frac_x=0, frac_y=0, frac_z=0;  
  double num_left=0, num=0, num_left_tot, num_tot;
...
/////////////////////////////////////////////////////////////////////////////////////////
      ...
      header1.time = 0.01;
      header1.redshift = 0;
      header1.BoxSize = 1.e-3;  //boxsize of 1.e-3 kpc, or 1 pc
...
//////////write new file!!!!!!!!!!!
      ...
      num_tot = (9./9.) * double(NumPart_new);
      num_left_tot  = (8./9.) * double(NumPart_new);

      ngrid_tot = pow(num_tot, 1./3.);
      ngrid_left_tot = pow(num_left_tot, 1./3.);
		...
		for(l=0;l<nnew;l++)
		  {
                      ...
                      num++;
                      frac_x = ngrid_x/ngrid_tot;
                      frac_y = ngrid_y/ngrid_tot;
                      frac_z = ngrid_z/ngrid_tot;
      ...
      for(k=0,pc_new=pc;k<6;k++)
	{
      ...
      massnew = 1000.0 * 1.e-10;  //total mass of cloud is 1000.0 pc
      massnew = massnew / nnew_doub / ( (double) header_old.npart[0]); 
      printf("total particles = %d, massnew = %lg\n", header_old.npart[0], massnew);
