...
//#define width_small 1.e2//which_sim==0
//#define width_small 0.0078125
#define width_small 1.0

#define height_z 0.015625

#define restart 0
#define write_freq 100

#define snapbegin 1 
//#define snapbegin 1600
...
#define neighbnum 10 
#define neighb_min 5  //which_sim==0
#define hfac_out 0.3
...
int main(int argc, char **argv)
{
  ...
  double fcor0, fcor1, dcor, dens_orig, dAdt[3], dAdt_part, B_exp, Bmag, error1, error2, ksum; 
  ...
  if(which_sim == 4)
    {
    sprintf(path, "/scratch/00863/minerva/vort_midres");
...
//// identify particles whose evolution shold be analytically followed
  ...
  if(which_sim < 4) xmax = ymax = zmax = header1.BoxSize/2.0;
  if(which_sim < 4) zmax = 0;
  ...
  for(i = 0; i < Ngas; i++)
     ...
     if(fabs(P[i].Pos[2]) < width_small && i%nskip == 0)
        {
        arrnum++;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(n=0; n<arrnum; n++)
      ...
      plist1[n].pos[0] = plist1[n].pos[1] = plist1[n].pos[2] = 0;
      //plist1[n].vel[0] = plist1[n].vel[1] = plist1[n].vel[2] = 0;
      for(p=0; p<3; p++)
        plist1[n].B_anal[p] = 0;
      }
/*
             ...
             //if(P[i].nh > nthresh_cur) 
               for(n=nmin; n<=nmax; n++)
                      ...
                      //f(P[i].Id == plist_id[n])
                         ...
                         if(n < 5)
                           printf("myrank = %d, j = %d, n = %d, dens = %lg\n", myrank, j, n, plist1[n].dens);
                         ...
                         //if(write_file == 1)
                           ...
                           //}
                         ncount++;
                         //} //end if Id == plist_id conditional
                      }
              //} //end P[n] for loop
...
/////////////////////////////////////////////////////////////////////////////////////////////////
//divide particle positions up into domains

int n_dom_x, n_dom_y, i_dom, j_dom;
double i_doub, j_doub, n_doub;

n_dom_x = (int) pow(tot_proc_sum, 0.5) * 1;
n_dom_y = n_dom_x;
n_doub = (double) n_dom_x;

if(myrank == 0) printf("n_dom_x = %d, n_dom_y = %d, n_doub = %lg\n", n_dom_x, n_dom_y, n_doub); 

int npart[n_dom_x][n_dom_y], npart_max=0;

for(i_dom=0; i_dom<n_dom_x; i_dom++)
  for(j_dom=0; j_dom<n_dom_y; j_dom++)
     npart[i_dom][j_dom] = 0;

for(i = 0; i < Ngas0; i++)
  {
  i_dom = (int) (P0[i].Pos[0] / header1.BoxSize * n_doub);
  j_dom = (int) (P0[i].Pos[1] / header1.BoxSize * n_doub); 
  //if(i%10000 == 0 && myrank == 0) 
  //  printf("i = %d, x = %lg, y = %lg, i_dom = %d, j_dom = %d\n", i, P0[i].Pos[0], P0[i].Pos[1], i_dom, j_dom);
  if(i_dom > n_dom_x-1) i_dom = n_dom_x-1;
  if(j_dom > n_dom_y-1) j_dom = n_dom_y-1;
  npart[i_dom][j_dom]++;
  //if(i%10000 == 0 && myrank == 0) 
  //  printf("i = %d, npart = %d\n", i, npart[i_dom][j_dom]);
  P0[i].x_dom = i_dom;
...
for(i_dom=0; i_dom<n_dom_x; i_dom++)
  for(j_dom=0; j_dom<n_dom_y; j_dom++)
    if(npart[i_dom][j_dom] > npart_max) npart_max = npart[i_dom][j_dom]; 

if(myrank == 0) printf("npart_max = %d \n", npart_max);

struct domain_data
  {
  double pos0[n_dom_x][n_dom_y][3], pos1[n_dom_x][n_dom_y][3], B[n_dom_x][n_dom_y][3];
  } *D;

if(!(D=(struct domain_data *) malloc((npart_max)*sizeof(struct domain_data))))
  {
   fprintf(stderr,"failed to allocate memory.\n");
   exit(0);
   }
...
for(i = 0; i < Ngas0; i++)
  ...
  j_dom = P0[i].y_dom;
  n = npart[i_dom][j_dom];
  for(p=0; p<3; p++) D[n].pos0[i_dom][j_dom][p] = P0[i].Pos[p]; 
  for(p=0; p<3; p++) D[n].pos1[i_dom][j_dom][p] = P[i].Pos[p];
  for(p=0; p<3; p++) D[n].B[i_dom][j_dom][p] = P0[i].B_anal[p];
  npart[i_dom][j_dom]++;
  }
...
for(i_dom=0; i_dom<n_dom_x; i_dom++)
  for(j_dom=0; j_dom<n_dom_y; j_dom++)
    {
    npart_sum = npart_sum + npart[i_dom][j_dom];
    //if(myrank == 0) printf("npart[%d][%d] = %d \n", i_dom, j_dom, npart[i_dom][j_dom]);
    }

if(myrank == 0) printf("npart_sum = %d \n", npart_sum);
/////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(j > 0)
         ...
         double Bx_min = 1.e20, By_min = 1.e20, Bz_min = 1.e20, hsm_min = 1.e20, hsm_max = -1.e20;
         double Bx_min_tot = 0, By_min_tot = 0, Bz_min_tot = 0, hsm_min_tot = 0, hsm_max_tot = 0;
         double Bx, By, Bz, Bx_tot=0, By_tot=0, Bz_tot=0;
         for(n=nmin; n<=nmax; n++)
            {
            ...
            if(plist0[n].hsm < hsm_min) hsm_min = plist0[n].hsm;
            if(plist0[n].hsm > hsm_max) hsm_max = plist0[n].hsm;
          ...
          MPI_Allreduce(&hsm_min, &hsm_min_tot, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
          MPI_Allreduce(&hsm_max, &hsm_max_tot, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
          if(myrank == 0) printf("Bx_min = %lg, By_min = %lg, Bz_min = %lg, hsm_min = %lg, hsm_max = %lg\n", 
                                  Bx_min_tot, By_min_tot, Bz_min_tot, hsm_min_tot, hsm_max_tot);
        

          double x0, y0, z0, x0_alt, y0_alt, z0_alt, x_alt, y_alt, z_alt, rad_alt, kernel_alt;
          double xmin, xmax, ymin, ymax;
          int imin, imax, jmin, jmax, ii, jj, id;
          for(n=nmin; n<=nmax; n++)
             {

             xmin = plist0[n].pos[0] - 3.*plist0[n].hsm; imin = (int) (xmin / header1.BoxSize * n_doub);
             xmax = plist0[n].pos[0] + 3.*plist0[n].hsm; imax = (int) (xmax / header1.BoxSize * n_doub);
             ymin = plist0[n].pos[1] - 3.*plist0[n].hsm; jmin = (int) (ymin / header1.BoxSize * n_doub); 
             ymax = plist0[n].pos[1] + 3.*plist0[n].hsm; jmax = (int) (ymax / header1.BoxSize * n_doub);
             ...
             i_dom = ii;  
             if(i_dom < 0)  i_dom = n_dom_x + i_dom; 
             if(i_dom > n_dom_x-1) i_dom = i_dom - (n_dom_x);
 
             j_dom = jj;
             if(j_dom < 0) j_dom = n_dom_y + j_dom; 
             if(j_dom > n_dom_y-1) j_dom = j_dom - (n_dom_y);

             if(i_dom < 0) {printf("n = %d, i_dom = %d, outside domain!i\n", n, i_dom); continue;}
             if(i_dom > n_dom_x-1) {printf("n = %d, i_dom = %d, outside domain!i\n", n, i_dom); continue;}
             if(j_dom < 0) {printf("n = %d, j_dom = %d, outside domain!i\n", n, j_dom); continue;}
             if(j_dom > n_dom_y-1) {printf("n = %d, j_dom = %d, outside domain!i\n", n, j_dom); continue;} 

             for(i = 0; i < npart[i_dom][j_dom]; i++)
                {
                 x0 = D[i].pos0[i_dom][j_dom][0];
                 y0 = D[i].pos0[i_dom][j_dom][1];
                 z0 = D[i].pos0[i_dom][j_dom][2];
                 x1 = D[i].pos1[i_dom][j_dom][0];
                 y1 = D[i].pos1[i_dom][j_dom][1];
                 z1 = D[i].pos1[i_dom][j_dom][2];
                 Bx = D[i].B[i_dom][j_dom][0];
                 By = D[i].B[i_dom][j_dom][1];
                 Bz = D[i].B[i_dom][j_dom][2];

                 rad = pow(((x0-plist0[n].pos[0])*(x0-plist0[n].pos[0]) + (y0-plist0[n].pos[1])*(y0-plist0[n].pos[1]) + (z0-plist0[n].pos[2])*(z0-plist0[n].pos[2])), 0.5);

                 rad_alt = 100.;
                 //if(P0[i].Pos[2] > height_z/2.) z_alt = P0[i].Pos[2] - height_z;
                 //if(P0[i].Pos[2] < height_z/2.) z_alt = P0[i].Pos[2] + height_z;

                 random = 0;
                 if(which_sim == 3) random = rand();

                 x0_alt = x0;  y0_alt = y0;  z0_alt = z0;
                 if(x0 + hfac*plist0[n].hsm > header1.BoxSize && plist0[n].pos[0] - hfac*plist0[n].hsm < 0)
                   x0_alt = x0 - header1.BoxSize;
                 if(y0 + hfac*plist0[n].hsm > header1.BoxSize && plist0[n].pos[1] - hfac*plist0[n].hsm < 0)
                   y0_alt = y0 - header1.BoxSize;
                 if(x0 - hfac*plist0[n].hsm < 0 && plist0[n].pos[0] + hfac*plist0[n].hsm  > header1.BoxSize)
                   x0_alt = x0 + header1.BoxSize;
                 if(y0 - hfac*plist0[n].hsm < 0 && plist0[n].pos[1] + hfac*plist0[n].hsm  > header1.BoxSize)
                   y0_alt = y0 + header1.BoxSize;

                 rad_alt = pow(((x0_alt-plist0[n].pos[0])*(x0_alt-plist0[n].pos[0]) + (y0_alt-plist0[n].pos[1])*(y0_alt-plist0[n].pos[1]) + (z0_alt-plist0[n].pos[2])*(z0_alt-plist0[n].pos[2])), 0.5);

                 x_alt = x1;  y_alt = y1;  z_alt = z1;
                 if(x1 + hfac*plist1[n].hsm > header1.BoxSize && plist1[n].pos[0] - hfac*plist1[n].hsm < 0)
                   x_alt = x1 - header1.BoxSize;
                 if(y1 + hfac*plist1[n].hsm > header1.BoxSize && plist1[n].pos[1] - hfac*plist1[n].hsm < 0)
                   y_alt = y1 - header1.BoxSize;
                 if(x1 - hfac*plist1[n].hsm < 0 && plist1[n].pos[0] + hfac*plist1[n].hsm  > header1.BoxSize)
                   x_alt = x1+ header1.BoxSize;
                 if(y1 - hfac*plist1[n].hsm < 0 && plist1[n].pos[1] + hfac*plist1[n].hsm  > header1.BoxSize)
                   y_alt = y1 + header1.BoxSize;
             
                  //if we have a 2D sim
                  //z0_alt = plist0[n].pos[2] + 1.e-10*plist1[n].hsm;
                  //z_alt  = plist1[n].pos[2] - 1.e-10*plist1[n].hsm;
             
                 if(random*1.0/RAND_MAX < 0.5 && (rad <= hfac*plist0[n].hsm || rad_alt <= hfac*plist0[n].hsm) && rad > hfac_in*plist0[n].hsm && plist1[n].NumNeighb < neighbnum)
                     {
                     dis_min = rad;
                     k = plist1[n].NumNeighb;
                     nb[n-nmin].pos0[0][k] = x0_alt;
                     nb[n-nmin].pos0[1][k] = y0_alt;
                     nb[n-nmin].pos0[2][k] = z0_alt;
                     //if(rad_alt < rad) nb[n-nmin].pos0[2][k] = z_alt;
                     nb[n-nmin].kernel0[k] = calc_kernel_spline(i, plist0[n].pos[0], plist0[n].pos[1], plist0[n]
.pos[2], nb[n-nmin].pos0[0][k], nb[n-nmin].pos0[1][k], nb[n-nmin].pos0[2][k], plist0[n].hsm, cosmo_fac);
                     //nb[n-nmin].rad[k] = rad;
                     plist1[n].NumNeighb++;

                     nb[n-nmin].pos1[0][k] = x_alt;
                     nb[n-nmin].pos1[1][k] = y_alt;
                     nb[n-nmin].pos1[2][k] = z_alt;
                     nb[n-nmin].kernel1[k] = calc_kernel_spline(i, plist1[n].pos[0], plist1[n].pos[1], plist1[n].pos[2], nb[n-nmin].pos1[0][k], nb[n-nmin].pos1[1][k], nb[n-nmin].pos1[2][k], plist1[n].hsm, cosmo_fac);
        
                     nb[n-nmin].B[0][k] = Bx;
                     nb[n-nmin].B[1][k] = By;
                     nb[n-nmin].B[2][k] = Bz;              
                     } //end if((rad <= hfac*plist0[n].hsm loop
                   }  //end for(i = 0; i < npart[i_dom][j_dom]; i++) loop
                } 
             }  //end for(n=nmin; n<=nmax; n++) loop
...
/* 
               for(p=0; p<3; p++)
                  ...
                  for(k=0; k<plist1[n].NumNeighb; k++)
                    {
                    nb[n-nmin].pos0[p][k] = nb[n-nmin].pos1[p][k] * pow(1 - delfac, -1);
                    }
                  }
                for(p=0; p<3; p++)
                  {
                  plist0[n].pos[p] = plist1[n].pos[p] * pow(1 - delfac, -1);
                  }
*/
...
/*
                   for(p=0; p<3; p++) 
                     if(plist1[n].pos[p] < nb[n-nmin].pos1[p][k] &&  plist0[n].pos[p] > nb[n-nmin].pos0[p][k]) 
                        nb[n-nmin].include[k] = 0;
*/
                   }
      ///////////////////////////////////////////////////////////////////////////
                 ...
                 fcor0 = fcor1 = 1;
                 plist0[n].dens = plist1[n].dens = error1 = error2 = ksum = 0;
                 ...
                 for(k=0; k<plist1[n].NumNeighb; k++)
                  ...
                  if(nb[n-nmin].include[k] > 0)
                    ...
                    //ksum = ksum + (4./3.*PI) * pow(plist0[n].hsm,3) * calc_kernel_spline(n, plist0[n].pos[0], plist0[n].pos[1], plist0[n].pos[2], nb[n-nmin].pos0[0][k], nb[n-nmin].pos0[1][k], nb[n-nmin].pos0[2][k], plist0[n].hsm, cosmo_fac);
                    ksum = ksum + P[1].Mass;
                    }
                  }  //end loop for density calculation
                 ksum = ksum / (4./3.*PI) *  pow(plist0[n].hsm,-3); //mass over volume
                 ksum = ksum / plist0[n].dens;  //ratio of mass/vol to calculated density

                 if(n == 1) printf("ksum = %lg\n", ksum); 
                 ...
                 for(k=0; k<plist1[n].NumNeighb; k++)
                  ...
                  if(nb[n-nmin].include[k] > 0)
                    ...
                    //if(n==1)
                    //  printf("Bx = %lg, By = %lg, Bz = %lg, Bsmoothx = %lg, Bmoothy = %lg, Bmoothz = %lg, nb_Bx = %lg, nb_By = %lg, nb_Bz = %lg\n", plist0[n].B_anal[0], plist0[n].B_anal[1], plist0[n].B_anal[2], plist0[n].Bsmooth[0], plist0[n].Bsmooth[1], plist0[n].Bsmooth[2], nb[n-nmin].B[0][k], nb[n-nmin].B[1][k], nb[n-nmin].B[2][k]);
                   //if (n==1) 
                   //  printf("x0 = %lg, y0 = %lg, z0 = %lg, x = %lg, y = %lg, z = %lg, x0_nb = %lg, y0_nb = %lg, z0_nb = %lg, x_nb = %lg, y_nb = %lg, z_nb = %lg\n", plist0[n].pos[0], plist0[n].pos[1], plist0[n].pos[2], plist1[n].pos[0], plist1[n].pos[1], plist1[n].pos[2], nb[n-nmin].pos0[0][k], nb[n-nmin].pos0[1][k], nb[n-nmin].pos0[2][k], nb[n-nmin].pos1[0][k], nb[n-nmin].pos1[1][k], nb[n-nmin].pos1[2][k]);
                    }
                 ...
                 //if(j > 1)  for(p=0; p<2; p++) plist0[n].B_anal[p] = plist0[n].Bsmooth[p];

                 if(n < 5) 
                    printf("n = %d, fcor0 = %lg, error1 = %lg, error2 = %lg, dwdu0[0] = %lg, dwdu0[1] = %lg, dwdu0[2] = %lg, ncheck_max = %d, ncheckLO[0] = %d, ncheckLO[1] = %d, ncheckLO[2] = %d, ncheckHI[0] = %d, ncheckHI[1] = %d, ncheckHI[2] = %d \n",  n, fcor0, error1, error2, nb[n-nmin].dwdu0[0][5], nb[n-nmin].dwdu0[1][5], nb[n-nmin].dwdu0[2][5], ncheck_max, ncheckLO[0], ncheckLO[1], ncheckLO[2], ncheckHI[0], ncheckHI[1], ncheckHI[2]);
                 //if(n < 5) 
                 ...
                 if(n < 5) printf("n = %d, det_jacob = %lg, jacob00=%lg, jacob01=%lg, jacob02=%lg, jacob10=%lg, jacob11=%lg, jacob22=%lg, jacob20=%lg, jacob21=%lg, jacob22=%lg \n", n, det_jacob, jacob[0][0], jacob[0][1], jacob[0][2], jacob[1][0], jacob[1][1], jacob[1][2], jacob[2][0], jacob[2][1], jacob[2][2]);
                ...
                for(p=0; p<3; p++)
                 ...
                 printf("n = %d, n_neighb = %d, n_neighb1 = %d, dens0 = %lg, dens1 = %lg, z0 = %lg, z1 = %lg, hsm0 = %lg, hsm1 = %lg, deform = %lg, det_jacob = %lg, detD_anal = %lg, ERROR = %lg, B_anal[0] = %lg, B_anal[1] = %lg, B_anal[2] = %lg\n",
                    n, plist1[n].NumNeighb, plist1[n].NumCalc, plist0[n].dens, plist1[n].dens, plist0[n].pos[2], plist1[n].pos[2], plist0[n].hsm, plist1[n].hsm, jacob_fin[0][0],
          ...
          if(write_file == 1)
            ...
            if(myrank == 0)
              for(i = 0; i < Ngas; i++)
                ...
                fwrite(&Ptot[i], sizeof(double), 1, outfile2);
                 //if(P[i].marker > 0)
                 //   printf("Ptot = %lg, nh = %lg, Bfieldz = %lg\n", Ptot[i], P[i].Rho, PBfieldz[i]);
                 //if(Ptot[i] < 1.e-30)
                 //   printf("UH OH Ptot = %lg, nh = %lg, Bfieldz = %lg\n", Ptot[i], P[i].Rho, PBfieldz[i]);
                }
...
*/
  ...
  //For NON-cosmological runs
  if(which_sim < 5) {Time = 1.0; zred = 0;}
...
double del_rho(double x_part, double y_part, double z_part, double x, double y, double z, double hsm, double rad_comp)
{
  double kernel, dkernel;
  double rad, ratio, drdu;
...
if(ktype == 2)
{
 ...
 if(ratio < 0.5)
  ...
  dkernel = sigma * dkernel * (rad/hsm/hsm) + sigma * kernel * (-3./hsm);
}
  return(dkernel);
}
