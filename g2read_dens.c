...
struct particle_data 
{
  ...
  float  HII, HeII,  HeIII, H2I, H2II, hsm, DII, HDI, gam, dummy;
} *P;
...
int main(int argc, char **argv)
{
  ...
  double nh, nhmax, mass, mmax, dis, xmax, ymax, zmax, sl, masstot, temp, tmax, h2, h2max, hdmax, heiimax, gam, gammin;
  ...
  sprintf(basename, "tgrid2l_g2shift");
  ...
  Ngas = load_snapshot(input_fname, files);
	  ...
	  mmax=0;
	    ...
	    for(i = 1; i <= Ngas; i++)
		{
	      ...
	      // if(temp > 1500.0 && nh > 1.e6)
		// printf("Id = %d gam = %g T=%g h2=%g nh=%g\n", gam, Id[i], temp, h2, nh);
	      ...
	      if(Id[i] == 4025757)
	       printf("Id = %d T=%g h2=%g nh=%g, gam=%g\n", Id[i], temp, h2, nh, P[i].gam);
