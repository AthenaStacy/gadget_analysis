...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  //int Idnew, nnew=1, nnew2=0;
  int Idnew, nnew=4, nnew2=1;
  double *pos, massnew, hsmnew, Unew, x, y, z, dis, nnew_doub, nnew2_doub;
  double randomx, randomy, randomz, nh, nhmax; 
  double ngrid_xleft=1, ngrid_yleft=1, ngrid_zleft=1, ngrid_x=1, ngrid_y=1, ngrid_z=1;
  double ngrid2_tot = 0, ngrid_tot = 0;
  double frac_xleft=0, frac_yleft=0, frac_zleft=0, frac_x=0, frac_y=0, frac_z=0;  
  double num_left=0, num=0, num_left_tot, num_tot, num2_tot;
  double dmax = 1.e10;  //size of 'cut-out' square in pc
  double mass_max = 1.e12;

  nnew_doub = (double) nnew;  nnew2_doub = (double) nnew2;

  int nnew_tot;
  nnew_tot = nnew + nnew2;

  double nnew_tot_doub;
  nnew_tot_doub = nnew_doub + nnew2_doub;
...
//////////write new file!!!!!!!!!!!
      ...
      num_tot =  double(NumPart_new*nnew);
      num2_tot = double(NumPart_new*nnew2);

      ngrid_tot = pow(num_tot, 1./3.);
      ngrid2_tot = pow(num2_tot, 1./3.);

      double frac_x, frac_y, frac_z, random_x, random_y, random_z;
                ...
                for(l=0;l<nnew2;l++)
                  {
                      ...
                      num++;
                      random_x = rand();  random_x = random_x / RAND_MAX;
                      random_y = rand();  random_y = random_y / RAND_MAX;
                      random_z = rand();  random_z = random_z / RAND_MAX;

                      x = header1.BoxSize*random_x / 2. + header1.BoxSize/4.; //fill each direction uniformly  
                      y = header1.BoxSize*random_y / 2. + header1.BoxSize/4.;
                      z = header1.BoxSize*random_z / 2. + header1.BoxSize/4.;

                      P[pc_new].Pos[0]=x;  P[pc_new].Pos[1]=y;  P[pc_new].Pos[2]=z;
		...
		for(l=0;l<nnew;l++)
		  {
                      ...
                      x = header1.BoxSize*random_x; //fill each direction uniformly  
                      y = header1.BoxSize*random_y;
                      z = header1.BoxSize*random_z;
	  ...
	  for(n=0;n<header_old.npart[k];n++)
	    {
		  ...
		  for(l=0;l<nnew_tot;l++)
		    {
                    massnew = P[pc_new].Mass/nnew_tot_doub; 
...
#if(BFF)
          double bwrite_x=1.e-20, bwrite_y=0, bwrite_z=0;
          double bwrite_x_cen = 2.e-20, bwrite_y_cen = 0, bwrite_z_cen = 0;
          double xmin, ymin, zmin, xmax, ymax, zmax, xpos, ypos, zpos, xcen, ycen, zcen, radius;
          xmin = ymin = zmin = 0.25*header1.BoxSize;
          xmax = ymax = zmax = 0.75*header1.BoxSize;
          xcen = ycen = zcen = 0.5*header1.BoxSize;
          printf("writing bfields!\n");

          WSKIP;
          for(n=0, pc_sph=pc; n<header1.npart[0];n++)
            {
              xpos = P[pc_sph].Pos[0]; ypos = P[pc_sph].Pos[1]; zpos = P[pc_sph].Pos[2];
              radius = pow((xpos-xcen)*(xpos-xcen) + (ypos-ycen)*(ypos-ycen) + (zpos-zcen)*(zpos-zcen), 0.5);
              if(P[pc_sph].disx < dmax && P[pc_sph].disy < dmax && P[pc_sph].disz < dmax && P[pc_sph].Mass < mass_max)
              {
              if(xpos > xmin && xpos < xmax && ypos > ymin && ypos < ymax && zpos > zmin && zpos < zmax)
              //if(radius < 0.5*header1.BoxSize)
                //for(l=0;l<nnew2;l++)
                  {
                  //printf("enhance the central b-field! x = %lg, y = %lg, z = %lg\n", xpos, ypos, zpos);
                  fwrite(&bwrite_x_cen, sizeof(double), 1, outfile);
                  fwrite(&bwrite_y_cen, sizeof(double), 1, outfile);
                  fwrite(&bwrite_z_cen, sizeof(double), 1, outfile);
                  pc_sph++;
                  } 
              else
                //for(l=0;l<nnew;l++)
                  {
                  //printf("write the un-enhanced the b-field!\n");
                  fwrite(&bwrite_x, sizeof(double), 1, outfile);
                  fwrite(&bwrite_y, sizeof(double), 1, outfile);
                  fwrite(&bwrite_z, sizeof(double), 1, outfile);
                  pc_sph++;
                  }
               }
               if(pc_sph % 10000 == 0) printf("pc_sph = %d\n", pc_sph);
              //pc_sph++;
           }
          WSKIP;
#endif
...
int allocate_memory(void)
{
  ...
  if(!(P=(struct particle_data *) malloc(10.*NumPart*sizeof(struct particle_data))))
    {
...
//  P--;   /* start with offset 1 */
  ...
  if(!(Id=(int *) malloc(10.*NumPart*sizeof(int))))
    {
      fprintf(stderr,"failed to allocate memory.\n");
      exit(0);
    }
