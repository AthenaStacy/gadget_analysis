...
#define width 1.e3
#define width_small 4.e2

#define restart 1
#define write_freq 100

#define snapbegin 900
#define snapend 1100
#define snapread 10
...
//// identify particles whose evolution shold be analytically followed
  ...
  xmax = header1.BoxSize;
  ymax = zmax = header1.BoxSize/2.0;
  ...
  for(i = 0; i < Ngas; i++)
     ...
     if(disx < width_small && disy < width_small/2.0 && disz < width_small/2.0)
        {
        arrnum++;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(j == snapnum-2 ||(snapshot_number%write_freq==0 && j > 1))
    {
  ...
  for(n=0; n<arrnum; n++)
      ...
      for(p=0; p<3; p++)
       ...
       if(P[i].nh > nhmax /*&& P[i].sink < 0.5*/)
        ...
        nthresh_cur = nthresh_cur/ncount_doub/1.e5;  
             ...
             if(j == snapnum-2 || (snapshot_number%write_freq==0 && j > 1))
                { 
                 if(P[i].marker > 0)
                   ...
                   PBfieldx[i] = B0 / (double) tot_proc_sum;
                   PBfieldy[i] = B0 / (double) tot_proc_sum;
                   PBfieldz[i] = B0 / (double) tot_proc_sum;
                   }
                 }
...
*/
                ...
                for(p=0; p<3; p++)
                 ...
                 if(n < 5) 
                 printf("n = %d, dens0 = %lg, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, x = %lg, y = %lg, z = %lg, detD = %lg, detD_anal = %lg, detD_alt = %lg. B_anal[0] = %lg\n",
                    n, plist0[n].dens, plist0[n].nh_anal, plist1[n].dens,  plist1[n].nh_anal, plist1[n].pos[0], plist1[n].pos[1], plist1[n].pos[2],
          ...
          if(j == snapnum-2 || (snapshot_number%write_freq==0 && j > 1))
            ...
            if(myrank == 0)
              ...
              if(snapshot_number > 9999)
                sprintf(output_fname2, "%s/%s_bfield_%05d", pathout, basename, snapshot_number);
