...
#define width_small 1.5e2  //for homog; full width in pc of cubic box of particles whose evolution will be tracked
//#define width_small 600.  //for sod

#define restart 0
#define write_freq 1


#define snapcheck 1  //for homolog collapse!
#define snapbegin 1
//#define snapbegin 24
#define snapend 98   
...
/*
#define snapcheck 1  //for sod shock!
#define snapbegin 1
#define snapend 2  
*/
...
double hsm_calibrate(int n, int nmin, double nu, double hfac_out, int mode);
...
int main(int argc, char **argv)
{
  ...
  double hfac = 1.0, hfac_in = 0.0, hfac_out = 1.0, neighb_typ = 80., ngb_count; 
  double time_fac, cosmo_fac, init_pow=1;
  ...
  double B0_100=1.e-14, B0, A0, rho_baryon;
...
//// identify particles whose evolution shold be analytically followed
  ...
  double width_smallx, width_smally, width_smallz;
  width_smallx = width_smally = width_smallz = width_small;  
  ...
  width_smallx = width_smallx*2.;
  width_smally = width_smally;
  width_smallz = width_smallz;
  }
  printf("myrank = %d, nhmax = %lg, xmax = %lg, ymax = %lg, zmax = %lg\n", myrank, nhmax, xmax, ymax, zmax);
  ...
  for(i = 0; i < Ngas; i++)
     ...
     if(disx < width_smallx/2.0 && disy < width_smally/2.0 && disz < width_smallz/2.0 && i%nskip == 0)
        {
        arrnum++;
...
printf("j=%d, snapshot_number = %d\n", j, snapshot_number);
  ...
  time_fac = 1.0;
   ...
   rho_baryon = P[1].Rho;
   if(myrank == 0)
      printf("B0 = %lg, zred = %lg rho_baryon=%lg mass_part = %lg P[i].Mass = %lg \n", B0, zred, rho_baryon, mass_part, P[1].Mass);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(n=0; n<arrnum; n++)
	...
	ncount_doub=0.;
             ...
             if(write_file == 1)
                { 
                 if(P[i].marker > 0)
                   ...
                   PAfieldx[i] = B0 * pow(P[i].Rho/rho_baryon, init_pow) * (P[i].Pos[1]-yshift) / (double) tot_proc_sum;
                   PAfieldy[i] = 0;
                   PAfieldz[i] = 0;
                   }
                 }
               ...
               for(n=nmin; n<=nmax; n++)
                      ...
                      if(P[i].Id == plist_id[n])
                         ...
                         if(j == 0)
                           ...
                           for(p=0; p<1; p++)
                             plist1[n].A_anal[p] =  B0 * pow(P[i].Rho/rho_baryon, init_pow) * (P[i].Pos[1]-yshift);
                           }
                          if(j == 0 && restart == 1)
                           {
        ...
        double x_alt, x_alt2, x_alt3, rad2, rad3;
        if(j > 0)
         {
          for(i = 0; i < Ngas0; i++)
            {
             ...
             for(n=nmin; n<=nmax; n++)
                {
                 ...
                 x_alt = P0[i].Pos[0];
                 if(plist0[n].pos[0] > header1.BoxSize - plist0[n].hsm && P0[i].Pos[0] < plist0[n].hsm)
                   {
                   x_alt = P0[i].Pos[0] + header1.BoxSize;
                   printf("SHIFT x = %lg, x0 = %lg, x_part = %lg\n", P0[i].Pos[0], x_alt, plist0[n].pos[0]);
                   }
                 if(plist0[n].pos[0] < plist0[n].hsm && header1.BoxSize - P0[i].Pos[0] < plist0[n].hsm)                
                   {
                   x_alt = P0[i].Pos[0] - header1.BoxSize;
                   printf("SHIFT x = %lg, x0 = %lg, x_part = %lg\n", P0[i].Pos[0], x_alt, plist0[n].pos[0]);
                   }

                 rad = pow(((x_alt-plist0[n].pos[0])*(x_alt-plist0[n].pos[0]) + (P0[i].Pos[1]-plist0[n].pos[1])*(P0[i].Pos[1]-plist0[n].pos[1]) + (P0[i].Pos[2]-plist0[n].pos[2])*(P0[i].Pos[2]-plist0[n].pos[2])), 0.5);
                 ...
                 //if(rad < hfac*plist0[n].hsm && disx < hfac*plist0[n].hsm && disy < hfac*plist0[n].hsm && disz < hfac*plist0[n].hsm && rad > hfac_in*plist0[n].hsm && plist1[n].NumNeighb < neighbnum)
                     {
                     if(n == 1) printf("rad=%lg, disx=%lg, disy=%lg, disz=%lg\n", rad, disx, disy, disz);
                     dis_min = rad;
                     k = plist1[n].NumNeighb;
                     nb[n-nmin].kernel0[k] = calc_kernel_spline(i, plist0[n].pos[0], plist0[n].pos[1], plist0[n].pos[2], x_alt, P0[i].Pos[1], P0[i].Pos[2], plist0[n].hsm, cosmo_fac);
                     kernel0 = 1;
                     //kernel0 = nb[n].kernel0[k];
                     nb[n-nmin].pos0[0][k] = x_alt * pow(kernel0,1./3.);
          ...
          for(i = 0; i < Ngas; i++)
             ...
             for(n=nmin; n<=nmax; n++)
               ...
               x_alt  = P[i].Pos[0];
               x_alt2 = P[i].Pos[0] + header1.BoxSize;
               x_alt3 = P[i].Pos[0] - header1.BoxSize;
 
               rad = pow(((x_alt-plist1[n].pos[0])*(x_alt-plist1[n].pos[0]) + (P[i].Pos[1]-plist1[n].pos[1])*(P[i].Pos[1]-plist1[n].pos[1]) + (P[i].Pos[2]-plist1[n].pos[2])*(P[i].Pos[2]-plist1[n].pos[2])), 0.5);
               rad2 = pow(((x_alt2-plist1[n].pos[0])*(x_alt2-plist1[n].pos[0]) + (P[i].Pos[1]-plist1[n].pos[1])*(P[i].Pos[1]-plist1[n].pos[1]) + (P[i].Pos[2]-plist1[n].pos[2])*(P[i].Pos[2]-plist1[n].pos[2])), 0.5);
               rad3 = pow(((x_alt3-plist1[n].pos[0])*(x_alt3-plist1[n].pos[0]) + (P[i].Pos[1]-plist1[n].pos[1])*(P[i].Pos[1]-plist1[n].pos[1]) + (P[i].Pos[2]-plist1[n].pos[2])*(P[i].Pos[2]-plist1[n].pos[2])), 0.5);

               if(rad < 1.5*hfac*plist1[n].hsm || rad2 < 1.5*hfac*plist1[n].hsm || rad3 < 1.5*hfac*plist1[n].hsm)
               for(k=0; k<plist1[n].NumNeighb; k++)
                  {
                  if(P[i].Id == nb[n-nmin].id[k])
                     {

                     if(nb[n-nmin].pos0[0][k] > header1.BoxSize)
                         {
                         x_alt = P[i].Pos[0] + header1.BoxSize;
                         printf("SHIFT x = %lg, x0 = %lg, x_part = %lg\n", P[i].Pos[0], x_alt, plist1[n].pos[0]);
                         }
                    if(nb[n-nmin].pos0[0][k] < 0)
                        {
                        x_alt = P[i].Pos[0] - header1.BoxSize;
                        printf("SHIFT x = %lg, x0 = %lg, x_part = %lg\n", P[i].Pos[0], x_alt, plist1[n].pos[0]);
                        }

                     nb[n-nmin].kernel1[k] = calc_kernel_spline(i, plist1[n].pos[0], plist1[n].pos[1], plist1[n].pos[2], x_alt, P[i].Pos[1], P[i].Pos[2], plist1[n].hsm, cosmo_fac);                          
                     kernel1 = 1;
                     //kernel1 = nb[n].kernel1[k];
                     nb[n-nmin].pos1[0][k] = x_alt * pow(kernel1,1./3.);
...
/////////////////////////////////////////////////////////////
            ...
            for(n=nmin; n<=nmax; n++)
                ...
                for(p=0; p<3; p++)
                 ...
                 if(n < 5) printf("fcor0 = %lg, dcor = %lg, dwdu0[0] = %lg, dwdu0[1] = %lg, dwdu0[2] = %lg\n", fcor0, dcor, nb[n-nmin].dwdu0[0][2], nb[n-nmin].dwdu0[1][2], nb[n-nmin].dwdu0[2][2]);
              
                 for(k=0; k<plist1[n].NumNeighb; k++)
                  {
                //  if(nb[n-nmin].kernel0[k]>0 /*&& nb[n-nmin].kernel1[k]>0*/)
                //     {
                     ...
                     for(p=0; p<3; p++)
                      ...
                      if(nb[n-nmin].id[k] == plist_id[n]) printf("BADNESS n = %d, dWdu_calc = %lg\n", n, dWdu_calc); 
                      for(q=0; q<3; q++)
                        {
                        A0 = plist0[n].A_anal[q];

                        //vel_fac1 = (plist1[n].pos[q] - nb[n-nmin].pos1[q][k]) ;
                        //vel_fac2 = (plist0[n].pos[q] - nb[n-nmin].pos0[q][k]) ;

                        vel_fac1 = (plist1[n].pos[q] - plist0[n].pos[q]) ;
                        vel_fac2 = (nb[n-nmin].pos1[q][k] - nb[n-nmin].pos0[q][k]) ;     

                        //vel_fac1 = -(plist0[n].vel[p] - nb[n-nmin].vel0[p][k]) * plist0[n].A_anal[q];
                        //vel_fac2 = -plist0[n].A_anal[p] * (plist0[n].vel[q] - nb[n-nmin].vel0[q][k]);
                        //vel_fac1 = vel_fac1 * vel_conv;
                        //vel_fac2 = vel_fac2 * vel_conv;

                        //dAdt_part = (P[1].Mass / plist1[n].dens) * A0 * vel_fac1 * dWdu1;
                        //dAdt_part = dAdt_part - (P[1].Mass / plist0[n].dens) * A0 * vel_fac2 * dWdu0;

                        dAdt_part = (P[1].Mass / dens_calc);
                        dAdt_part = dAdt_part * A0 * (vel_fac1 - vel_fac2) * dWdu_calc;
                     
                        if(n == 1 && p == 0) printf("k = %d, dAdt[%d] = %lg, dAdt_part = %lg, dwdu0[0] = %lg, dwdu[1] = %lg, dwdu0[2] = %lg, vel_fac1 = %lg, vel_fac2 = %lg\n", k, p, dAdt[p], dAdt_part, nb[n-nmin].dwdu0[0][k], nb[n-nmin].dwdu0[1][k], nb[n-nmin].dwdu0[2][k], vel_fac1, vel_fac2);
                 ...
                 if(plist0[n].A_anal[0] != plist0[n].A_anal[0])
                   ...
                   for(p=0; p<1; p++)
                    plist0[n].A_anal[p] = B0 * pow(plist0[n].dens/rho_baryon, 0) * P[i].Pos[1];
                   }

                 if(dAdt[0] != dAdt[0])
                   {
                   printf("FIX THE A-FIELDS AGAIN!, dAdt[2] = %lg\n", dAdt[2]);
                   for(p=0; p<3; p++)
                     dAdt[p] = 0;
                   }
                 ...
                 if(n < 5) 
                 {
                 A_exp = plist0[n].A_anal[0]*pow(1-delfac,-1); 
                 dens_exp = plist0[n].dens * pow(1-delfac,-3);
                 printf("n = %d, n_neighb0 = %d, n_neighb1 = %d, dens0 = %lg, dens1 = %lg, dAdt[0] = %lg, dAdt[1] = %lg, dAdt[2] = %lg, fcor1 = %lg, A_anal0[0] = %lg, A_anal0[1] = %lg, A_anal0[2] = %lg hsm0 = %lg, hsm1 = %lg, vel_fac = %lg, dis_fac = %lg, dWdu0 = %lg, A_exp = %lg, kernel = %lg,  A_anal[0] = %lg, A_anal[1] = %lg, ERROR = %lg\n",
                    n, plist1[n].NumNeighb, neighb1, plist0[n].dens, plist1[n].dens, dAdt[0], dAdt[1], dAdt[2], fcor1, 
...
do{
  ...
  dens_new = P[1].Mass * ngb_count / (4./3. * PI) / pow(hfac_out*hsm_new, 3);

  //if(n == 1) printf("hsm_old = %lg, hsm_new = %lg, dens_old = %lg, dens_sum = %lg, dens_new = %lg \n", hsm_old, hsm_new, dens_old, dens_sum, dens_new);

  }while( fabs(hsm_new - hsm_old)/hsm_new  > epsilon ) ;
