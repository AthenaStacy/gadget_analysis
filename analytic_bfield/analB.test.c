...
int main(int argc, char **argv)
{
  ...
  int  ID, i, k=0, ksort, j, n, p, q, r, s, type, snapshot_number, files, Ngas, Ngas0, random, ncount, ncounthalo1;
  double x,y,z,x1,y1,z1, delr, arrnum_doub = 10.0, neighbnum_doub, hfac = 0.75, hfac_in = 0.01, hfac_avg, mfac = 0.0, delfac = 1.e-3, cosmo_fac, time_fac;
  ...
  double xfac, yfac, zfac, kfloat, nh_avg[snapnum], nh_avg2, num_tot, omega, omega0, nthresh;
  ...
  sprintf(path, "/work/00863/minerva/iso_analB");
  sprintf(basename, "iso_map");
  sprintf(basename2, "iso_analB");
             ...
             if(P[i].nh > 2.e-2) 
               for(n=0; n<arrnum; n++)
                      ...
                      if(P[i].Id == plist_id[n])
                         ...
                         if(j == 0)
                           {
                           plist1[n].nh_anal = P[i].nh;
                           plist1[n].nh_anal_alt = plist1[n].dens_alt = P[i].nh;
                           printf("j = %d, n = %d, nh_anal1 = %lg\n", j, n, plist1[n].nh_anal);
                           for(p=0; p<3; p++)
                             plist1[n].B_anal[p] = 1.e-20;
                           }
                         }
                      }
              }
...
*/
            ...
            for(n=0; n<arrnum; n++)
                ...
                for(k=0; k<plist1[n].NumNeighb-1; k++)
                  ...
                  for(p=0; p<3; p++)
                   { 
                    if(fabs(plist0[n].pos[p] - nb[n].pos0[p][k]) < hfac_avg*plist0[n].hsm && dis0[k] < hfac*plist0[n].hsm)  
                    ...
                    //if(dis0[k] < 0.5*hfac*plist0[n].hsm)             //alt2
                     ...
                     if(fabs(plist1[n].pos[p] - nb[n].pos1[p][k]) < hfac_avg*plist1[n].hsm && dis0[k] < hfac*plist0[n].hsm)
                ...
                for(p=0; p<3; p++)
                 ...
                 plist1[n].dens_alt = plist0[n].dens_alt*pow(1.-delfac,-3);

                 for(p=0; p<3; p++)
                   plist1[n].B_anal[p] = plist0[n].B_anal[p] * (1. + nb[n].deform[p][p][k]) / plist1[n].detD;
                 
                 printf("n = %d, dens0 = %lg, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, hsm0 = %lg, hsm1 = %lg, deform = %lg, detD = %lg, detD_anal = %lg, detD_alt = %lg\n",
                    n, plist0[n].dens, plist0[n].nh_anal, plist1[n].dens,  plist1[n].nh_anal, plist0[n].hsm, plist1[n].hsm, nb[n].deform[0][0][0],
                    plist0[n].dens/plist1[n].dens, plist1[n].detD, plist0[n].dens_alt/plist1[n].dens_alt);
