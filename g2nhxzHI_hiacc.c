...
//    int n_grid2[N_grid][N_grid];
        ...
        if(m < 10)
          {
            ...
            sprintf(buf, "%s_000%d.nhxz", dir, m);
	...
	width= 50*4.85e-4*header.HubbleParam/(1.e3*header.time);  //500 AU   
        ...
        for(n = 0; n < N_gas; n++)
         ...
         xCOM = yCOM = zCOM = vxCOM = vyCOM = vzCOM = ncount_doub = nhmax = mmax = 0;
         for(n=0;n < N_gas; n++) 
            {
             nh = P[n].nh;
             //if(nh > nhmax)
