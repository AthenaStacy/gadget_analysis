...
int main(int argc, char **argv)
{
  ...
  sprintf(output_fname, "/work/00863/minerva/bin_MR10_ideal_000");
...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  int Idnew;
  int nskip=8;
  double *pos, massnew, hsmnew, x, y, z, dis, nnew=0.125;
  double randomx, randomy, randomz, nh, nhmax;
  double dmax = 1.e6;  //size of 'cut-out' square in pc
  double mass_max = 1.e-12;
      ...
      for(k=0,pc_new=pc;k<6;k++)
        {
          for(n=0;n<header1.npart[k];n++)
            {
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax && pc_new%nskip == 0)  
                NumPart_new++;
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax && k ==0 && pc_new%nskip == 0 && P[pc_new].Mass < mass_max)
                Ngas_new++;
              pc_new++;
            }
        }
...
//divergence between gadget1 and gadget2 begins here!
		...
		if(pc_sph%nskip == 0)
		  {
		   hsmnew = P[pc_sph].hsm*pow(nnew,-.3333); 
