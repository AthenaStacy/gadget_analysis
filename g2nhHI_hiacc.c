...
//    int n_grid2[N_grid][N_grid];
    ...
    double entries = 0.0;
    double temp_pos = 0.0;
    //double min = 1.e6;       //bin_HR density range
    //double max = 1.e12;
    double min = 1.e9;     //hires density range  
    double max = 1.e14;
    double abs_min = min;
    double abs_max = max;
    ...
    double nh, nhmax, mmax, xmax, ymax, zmax, vx, vy, vz, xCOM, yCOM, zCOM, vxCOM, vyCOM, vzCOM, ncount_doub;
    ...
    sprintf(dir, "/nobackupp7/astacy/ifront_hiacc");
	...
	width= 50*4.85e-4*header.HubbleParam/(1.e3*header.time);  //cut out central 500 AU   
        slice= 50*4.85e-4*header.HubbleParam/(1.e3*header.time);  

        //width= 10e0*header.HubbleParam/(1.e3*header.time);
        //slice= 10e0*header.HubbleParam/(1.e3*header.time);
        ...
        for(n = 0; n < N_gas; n++)
         ...
         xCOM = yCOM = zCOM = vxCOM = vyCOM = vzCOM = ncount_doub = nhmax = mmax =mmax =  0;
         for(n=0;n < N_gas; n++)
            {
             nh = P[n].nh;
             //if(nh > nhmax)
