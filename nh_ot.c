...
//    int n_grid2[N_grid][N_grid];
    ...
    double min = 0.5;
    double max = 1.5;
    if(do_vel == 1) {min = 1.e-1; max = 100; /*max = 1.5e0;*/}
    double abs_min = min;
    double abs_max = max;
        ...
        for(n = 0; n < N_gas; n++)
          {
            ...
            if (n%100000 ==0) printf("dens = %lg, velx = %lg, vely = %lg, velz = %lg U = %lg\n", P[n].nh, P[n].Vel[0], P[n].Vel[1], P[n].Vel[2], P[n].U);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////// 
        ...
        double KE = 0, ntot, nhtot=0, nmean, nmin=100, nmax, nrms, vmin = 100, vmax = 0;
        ntot = double (N_gas);
        for(n=0;n<N_gas;n++)
          {
          KE = KE + 0.5*P[n].Mass*(P[n].Vel[0]*P[n].Vel[0] + P[n].Vel[1]*P[n].Vel[1] + P[n].Vel[2]*P[n].Vel[2]);          
          nhtot = nhtot + pow(P[n].nh,2);
          if(P[n].nh > nmax) nmax = P[n].nh;
          if(P[n].nh < nmin) nmin = P[n].nh;     
          if(P[n].Vel[1] > vmax) vmax = P[n].Vel[1];
          if(P[n].Vel[1] < vmin) vmin = P[n].Vel[1];
          }

        nmean = nhtot / ntot;
        nrms = pow(nmean, 0.5);

        printf("nrms = %lg nmin = %lg, nmax = %lg\n", nrms, nmin, nmax);
        printf("vmin = %lg, vmax = %lg\n", vmin, vmax);
