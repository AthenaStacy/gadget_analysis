...
int main(int argc, char **argv)
{
  ...
  int  i, j, jnum, k, arrnum=200, halo, n, nsink, nsink_bin, type, snapshot_number, files, Ngas, random, ncount, pair_count, nsinkmax=1000, idmax;
  ...
  if(halo == 11)
     {
     sprintf(basename, "bin_HRE7");
     sprintf(basenameout, "snapbin_HRE7");
     jnum = 99;
     }
  if(halo == 12)
     {
     sprintf(basename, "bin_HRE10");
     sprintf(basenameout, "snapbin_HRE10");
     jnum = 90;
     }
  if(halo == 13)
     {
     sprintf(basename, "bin_HRL7");
     sprintf(basenameout, "snapbin_HRL7");
     jnum = 19;
     }
  if(halo == 14)
     {
     sprintf(basename, "bin_HRL10");
     sprintf(basenameout, "snapbin_HRL10");
     jnum = 18;
     }
  if(halo == 15)
     {
     sprintf(basename, "hires_test10b");
     sprintf(basenameout, "snaphires_test10b");
     jnum = 2700;
     }
...
;  //convert to km/s
             ...
             if(P[n].sink > 0 /*|| P[n].nh > 1.0e13*/)
               {
               posx[nsink] = P[n].Pos[0]; 
               posy[nsink] = P[n].Pos[1];
               posz[nsink] = P[n].Pos[2];
               velx[nsink] = P[n].Vel[0];
               vely[nsink] = P[n].Vel[1];
               velz[nsink] = P[n].Vel[2];
               sinkmass[nsink] = P[n].Mass/1.e-10/.7;
               sinkid1[nsink] = Id[n];
               sinkid2[nsink] = 0;
               printf("nsink = %d, Mass = %g sink = %g ID = %d nh = %lg Dens = %lg Temp = %lg disAU = %lg, vel = %lg, vrad = %lg, vrot = %lg\n", nsink, P[n].Mass, P[n].sink, Id[n], P[n].nh, P[n].Rho, P[n].Temp, disAU, vel, vrad, vrot);
               nsink++;
               ...
               vesc = pow(6.67e-8*m_enc*2e33/(disAU*1.5e13),0.5)/1.e5;

               outfile=fopen(output_fname2, "a");
               fprintf(outfile, "%8d  %15.11g %15.11g %15.11g  %15.11g %15.11g\n", Id[n], P[n].Mass/1.e-10/.7, disAU, vrot, vrad, vesc);
               fclose(outfile);

               outfile=fopen("bin_vel.dat", "a");
               fprintf(outfile, "%8d  %15.11g %15.11g %15.11g  %15.11g %15.11g\n", Id[n], P[n].Mass/1.e-10/.7, disAU, vrot, vrad, vesc);
               fclose(outfile);
               }
   ...
   for(i=0; i<nsinkmax; i++)
	  ...
	  disCOM1=disCOM1*206264.806;
          ...
          //printf("Mass = %g ID = %d disAU = %lg, dCOM = %lg, vel = %lg, vrad = %lg, vrot = %lg, PE = %lg, KE = %lg\n", sinkmass[i], sinkid2[i], disAU, disCOM, vel, vrad, vrot, pe, ke);
          //printf("Mass = %g ID = %d disAU = %lg, dCOM1 = %lg, vel = %lg, vrad = %lg, vrot = %lg, PE = %lg, KE = %lg\n", sinkmass[ncount], sinkid2[ncount], disAU, disCOM1, vel, vrad, vrot, pe, ke);
   ...
   for(n=0; n<=nsinkmax-1; n++)
      {
      printf("%8d %8d %8d %8d %15.11g %15.11g %15.11g %15.11g %15.11g\n", id1[n], id2[n], id3[n], id4[n], a[n], dis_cur[n], mass1[n], mass2[n], vrot_sink[n]);
      fprintf(outfile, "%8d %8d %8d %8d %15.11g %15.11g %15.11g %15.11g %15.11g\n", id1[n], id2[n], id3[n], id4[n], a[n], dis_cur[n], mass1[n], mass2[n], vrot_sink[n]);
      }
   fclose(outfile);

   outfile=fopen("bin_pairs.dat", "a");
   for(n=0; n<=nsinkmax-1; n++)
      {
      if(id1[n] > 0)
        fprintf(outfile, "%8d %8d %8d %8d %15.11g %15.11g %15.11g %15.11g %15.11g\n", id1[n], id2[n], id3[n], id4[n], a[n], dis_cur[n], mass1[n], mass2[n], vrot_sink[n]);
