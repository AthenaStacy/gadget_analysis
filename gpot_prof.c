...
int main(int argc, char **argv)
{
  ...
  int  i, j, jnum, k, arrnum=200, halo, n, nsink, nsink_bin, type, snapshot_number, files, Ngas, random, ncount, pair_count, idmax;
  ...
  double m_enc, m_enc_arr[arrnum], d_arr[arrnum], n_arr[arrnum], num_arr[arrnum], num2_arr[arrnum], turb, turb_arr[arrnum], c_s_arr[arrnum], vrad_arr[arrnum], vrot_arr[arrnum];
  double velx, vely, velz;
  double vel_arr[arrnum], velx_arr[arrnum], vely_arr[arrnum], velz_arr[arrnum], vradx_arr[arrnum], hd_arr[arrnum], nh_arr[arrnum], h2_arr[arrnum], elec_arr[arrnum], temp_arr[arrnum], temp_nh_arr[arrnum];
  ...
  sprintf(output_fname, "%s_gas_%03d.dat",basename, snapshot_number);
  sprintf(output_fname2, "%s_gas_nh_%03d.dat",basename, snapshot_number);
  ...
  nthresh = 2.e11;
   ...
   for(n=0; n<arrnum; n++)
     {
     m_enc_arr[n] = num_arr[n] = num2_arr[n] = nh_arr[n] = temp_arr[n] = temp_nh_arr[n] = h2_arr[n] = elec_arr[n] = hd_arr[n] = 0.0;
  ...
  for(n=0;n<=Ngas;n++) {
        ...
        for(i = 1; i < NumPart; i++)
             ...
             if(disAU < 5.e8)
               ...
               for(n=0; n<arrnum; n++)
                  {
                  if(disAU < d_arr[n+1] /*|| P[i].sink > 0.5*/)
                     m_enc_arr[n] = m_enc_arr[n] + P[i].Mass*1.e10/hubble;
                  }
               }
         ...
         for(n = 1; n <= Ngas; n++)
             ...
             if(P[n].nh > nmin && disAU > 0 && P[n].sink > -4)
               ...
               if(disAU < d_arr[0])
                     ...
                     //nh_arr[0] = 2e33*(m_enc_arr[k] - m_enc_arr[k-1])/volume/(1.22*1.67e-24);
               ...
               for(k=0; k<arrnum; k++)
                  ...
                  //if(disAU < d_arr[k+1] && disAU > d_arr[k-1])
                     ...
                     nh_arr[k] = 2.e33*(m_enc_arr[k+1] - m_enc_arr[k])/volume/(1.22*1.67e-24);
