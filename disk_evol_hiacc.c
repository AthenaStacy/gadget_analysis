...
int main(int argc, char **argv)
{
  ...
  double warm_mass, warm_rad, warm_rmax, hii_frac, warm_temp, warm_dens;
  FILE *outfile;
  ...
  nhmax = warm_mass = warm_rad = warm_rmax = warm_temp = warm_dens = hii_frac = 0;
  mmax = mh_mass = 0;
  xmax = ymax = zmax = 0;
  ...
  for(n=0;n<Ngas;n++) { 
        ...
        for(i = 0; i < NumPart; i++)
         ...
         for(n = 0; n < Ngas; n++)
             ...
             if(P[n].Temp > 2000. && P[n].nh > 1.e5)
              {
              warm_mass = warm_mass + P[n].Mass * 1.e10 / 0.7;
              warm_rad  = warm_rad  + dis * (P[n].Mass * 1.e10 / 0.7);
              warm_temp = warm_temp + P[n].Temp * (P[n].Mass * 1.e10 / 0.7);
              warm_dens = warm_dens + P[n].nh * (P[n].Mass * 1.e10 / 0.7);
              hii_frac = hii_frac + P[n].HII * (P[n].Mass * 1.e10 / 0.7);
              if(dis > warm_rmax) warm_rmax = dis;              
              }
...
;  //convert to km/s
   ...
   if(warm_mass > 0)
     {
     warm_rad = warm_rad / warm_mass;  
     warm_temp = warm_temp / warm_mass;
     warm_dens = warm_dens / warm_mass;
     hii_frac = hii_frac / warm_mass;
     }
   else
    {
    warm_mass = 1.e-30;
   ...
   fprintf(outfile, "%15.10g %15.6g %15.6g %15.6g %15.6g %15.6g %15.6g\n", Time, warm_mass, warm_rad, warm_rmax, hii_frac, warm_temp, warm_dens); 
   fclose(outfile);
