...
#define width_small 1.e2  //full width in pc of cubic box of particles whose evolution will be tracked
...
int main(int argc, char **argv)
{
  ...
  double x,y,z,x1,y1,z1, delr, neighbnum_doub, nthresh = 1.e2, nthresh_cur, delfac = 1.e-2; 
...
//// identify particles whose evolution shold be analytically followed
  ...
  for(i = 0; i < Ngas; i++)
     ...
     if(disy < width_small/2.0 && disz < width_small/2.0 && i%nskip == 0)
        {
        arrnum++;
...
/////////////////////////////////////////////////////////////
            ...
            for(n=nmin; n<=nmax; n++)
                ...
                for(k=0; k<plist1[n].NumNeighb-1; k++)
                  ...
                  if(dis0[k] < hfac*plist0[n].hsm && nb[n-nmin].kernel0[k]>0 &&  nb[n-nmin].kernel1[k]>0)
                     {
                     for(p=0; p<3; p++)
                      { 
                       if(fabs(plist0[n-nmin].pos[p] - nb[n-nmin].pos0[p][k]) < hfac_avg*plist1[n-nmin].hsm)  
                         {
                         kernel_fac = nb[n-nmin].kernel0[k];
                         kernel_fac = 1;
                         //kernel_fac = pow(kernel_fac, 1) * pow(plist0[n].hsm,3);
                         davgLO0[p] = davgLO0[p] + fabs(plist0[n].pos[p] - nb[n-nmin].pos0[p][k]);
                         numLO0[p] = numLO0[p] + 1.0*kernel_fac; 
                         }
                        if(fabs(plist1[n].pos[p] - nb[n-nmin].pos1[p][k]) < hfac_avg*plist1[n].hsm)
                         {
                         kernel_fac = nb[n-nmin].kernel1[k];
                         kernel_fac = 1;
                         //kernel_fac = pow(kernel_fac, 1) * pow(plist1[n].hsm,3);
                         davgLO1[p] = davgLO1[p] + fabs(plist1[n].pos[p] - nb[n-nmin].pos1[p][k]);
                         numLO1[p] = numLO1[p] + 1.0*kernel_fac;
                         }                     
                       if(fabs(plist0[n].pos[p] - nb[n-nmin].pos0[p][k]) > hfac_avg*plist1[n].hsm)
                         {
                         kernel_fac = nb[n-nmin].kernel0[k];
                         kernel_fac = 1;
                         //kernel_fac = pow(kernel_fac, 1) * pow(plist0[n].hsm,3);
                         davgHI0[p] = davgHI0[p] + fabs(plist0[n].pos[p] - nb[n-nmin].pos0[p][k]);
                         numHI0[p] = numHI0[p] + 1.0*kernel_fac;
                         }
                        if(fabs(plist1[n].pos[p] - nb[n-nmin].pos1[p][k]) > hfac_avg*plist1[n].hsm)
                         {
                         kernel_fac = nb[n-nmin].kernel1[k];
                         kernel_fac = 1;
                         //kernel_fac = pow(kernel_fac, 1) * pow(plist1[n].hsm,3);
                         davgHI1[p] = davgHI1[p] + fabs(plist1[n].pos[p] - nb[n-nmin].pos1[p][k]);
                         numHI1[p] = numHI1[p] + 1.0*kernel_fac;
                         } 
                      }
                   }
                 }                
                ...
                for(p=0; p<3; p++)
                   for(q=0; q<3; q++)
                     ...
                     //deform_avg[p][q] =  (-(davgHI0[p] - davgLO0[p]) + (davgHI1[p] - davgLO1[p]))/(davgHI1[q] - davgLO1[q]);
                     if(n < 5) printf("n = %d, deform[%d][%d] = %lg\n", n, p, q, deform_avg[p][q]);
                     }
                 ...
                 for(p=0; p<3; p++)
                   {
                   if(numHI1[p] <= neighb_min || numHI0[p] <= neighb_min)
                     {
                     plist1[n].detD = 1;
                     for(q=0; q<3; q++) deform_avg[p][q] = 0;
                     }
                   if((numHI1[p] <= neighb_min || numHI0[p] <= neighb_min) && plist0[n].dens > 0 && plist1[n].dens > 0)
                 ...
                 plist1[n].B_anal[0] = plist0[n].B_anal[0] * (1. + deform_avg[0][0]) + plist0[n].B_anal[1] * deform_avg[0][1] + plist0[n].B_anal[2] * deform_avg[0][2]; 
                 plist1[n].B_anal[1] = plist0[n].B_anal[1] * (1. + deform_avg[1][1]) + plist0[n].B_anal[0] * deform_avg[1][0] + plist0[n].B_anal[2] * deform_avg[1][2];
                 plist1[n].B_anal[2] = plist0[n].B_anal[2] * (1. + deform_avg[2][2]) + plist0[n].B_anal[0] * deform_avg[2][0] + plist0[n].B_anal[1] * deform_avg[2][1];
...
matrix[0][0] = matrix00 + 1;
matrix[0][1] = matrix01;
matrix[0][2] = matrix02;
matrix[1][0] = matrix10;
matrix[1][1] = matrix11 + 1;
matrix[1][2] = matrix12;
matrix[2][0] = matrix20;
matrix[2][1] = matrix21;
matrix[2][2] = matrix22 + 1;
