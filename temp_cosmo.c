...
//    int n_grid2[N_grid][N_grid];
    ...
    sprintf(endtype, ".Temp");
    if(rotate == 1) sprintf(endtype, ".Tempxz");
        ...
        for(n = 0; n < N_gas; n++)
          {
            ...
            if(muh2 >= 1.22) mu=muh2;

            gamma = P[n].gam;

            P[n].Temp = P[n].U*unit_energy/unit_mass*mu*(gamma-1.0)*m_H/k_B;

            if (n%1000000 ==0) printf("temp = %lg\n", P[n].Temp);
