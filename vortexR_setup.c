...
for(j=0; j<=0; j=j+1)
  ...
  delx = dely = delz = 0;  //del's used in making bin_zoom10_cut_??? (bin_zoom10_???)
...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  double random_x, random_y, random_z, nh, nhmax; 
  double ngrid_xleft=1, ngrid_yleft=1, ngrid_zleft=1, ngrid_x=1500, ngrid_y=1500, ngrid_z=1;
  double ngrid_tot = 0;
      ...
      for(i=0; i<ngrid_x_int; i++)
         for(j=0; j<ngrid_y_int; j++)
             for(k=0; k<ngrid_z_int; k++)
                 ...
                 x = random_x / header1.BoxSize;
                 y = random_y / header1.BoxSize;
	  ...
	  for(n=0;n<header1.npart[k];n++)
	    {
              ...
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax && k ==0 && P[pc_new].Mass < mass_max)
                {
                //instead of evolving vel, replace this with initizalizing and evolving B-field
                P[pc_new].Vel[0] = 1.e-20;  //Bx = 1.e-20 Gauss ...
                P[pc_new].Vel[1] = 0;
                P[pc_new].Vel[2] = 0; 
