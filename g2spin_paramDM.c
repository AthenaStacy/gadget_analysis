...
int main(int argc, char **argv)
{
  ...
  int IDmax;
  double rho_b, rho_DM[3000], rho_vir, r_vir, v_vir;
  ...
  double m_enc_arr[3000], d_arr[3000], mass_enc;
  double KE, KE_tot, PE, PE_tot, vel, spin_param;
  int ID_sink1, ID_sink2, ID_sink3, ID_sink4, sinknumk, N_dm;
  FILE *outfile;
  FILE *infile;

  sprintf(path, "/work/00863/minerva/bf");
  sprintf(basename, "bf3_hs");
  ...
  ID_sink3 = 4653020;
       ...
       delx = sinkposx1 - header1.BoxSize/2.0;
       dely = sinkposy1 - header1.BoxSize/2.0;
       delz = sinkposz1 - header1.BoxSize/2.0;
       for(i = 0; i < NumPart; i++)
          {
          P[i].Pos[0] = P[i].Pos[0] - delx;
          P[i].Pos[1] = P[i].Pos[1] - dely;
          P[i].Pos[2] = P[i].Pos[2] - delz;
          if(P[i].Pos[0] < 0.0)
             P[i].Pos[0]=P[i].Pos[0]+header1.BoxSize;    
          if(P[i].Pos[1] < 0.0)
             P[i].Pos[1]=P[i].Pos[1]+header1.BoxSize;
          if(P[i].Pos[2] < 0.0)
             P[i].Pos[2]=P[i].Pos[2]+header1.BoxSize;
          if(P[i].Pos[0] > header1.BoxSize)
             P[i].Pos[0]=P[i].Pos[0]-header1.BoxSize;
          if(P[i].Pos[1] > header1.BoxSize)
             P[i].Pos[1]=P[i].Pos[1]-header1.BoxSize;
          if(P[i].Pos[2] > header1.BoxSize)
             P[i].Pos[2]=P[i].Pos[2]-header1.BoxSize; 
          }
         ...
         for(i = Ngas; i < NumPart; i++)
           ...
           for(i = 0; i < Ngas; i++)
              ...
              if(disAU < r_vir)
                ...
                h2_avg = h2_avg + P[i].H2I;
                if(P[i].nh > 1.e1 && P[i].Temp < 1.e3)
                  sfgas_avg = sfgas_avg + P[i].Mass/1.e-10/.7;
                ncount++;
                }
              }
             ...
             temp = temp/ncount;
             nh_avg = nh_avg/ncount;
             h2_avg = h2_avg/ncount;
             outfile=fopen("bf3_hs_sf","a");
             //fprintf(outfile, "%15.11g %15.11g %15.11g\n", Time, temp, nh_avg);
             fprintf(outfile, "%15.11g %15.11g %15.11g %15.11g %15.11g\n", Time, h2_avg, sfgas_avg, num_tot*1.e10/.7, ncount*9.0);
             fclose(outfile);

/*
             outfile=fopen("bf_hs_spin", "a");             
