...
//    int n_grid2[N_grid][N_grid];
    ...
    double entries = 0.0;
    double temp_pos = 0.0;
    double min = 1.e-21;     //hires bfield range  
    double max = 1.e-15;
    double abs_min = min;
    double abs_max = max;
...
//Read in B-field info!///////////////////////////////////////////////////////////
  ...
  sprintf(path2, "/nobackup/astacy/orion/");
  sprintf(basename2, "bin_zoom10");

  if(which_sim == 4)
   {
   sprintf(path, "/work/00863/minerva/binHR/");
   if(convert_vpot == 1) sprintf(path2, "/nobackup/astacy/orion/bfield_comp_vpot");
