...
int load_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double boxsize);
int reordering(void);
...
int main(int argc, char **argv)
{
  ...
  sprintf(basename, "lowresb_ifront3");
  sprintf(basenameout, "lowresb_ifront3_shift");
  ...
  boxsize=140.0;
  delx = 1.382712231 - (boxsize/2.e0);
  dely = 67.81652843 - (boxsize/2.e0);
  delz = 45.25685665 - (boxsize/2.e0);

  sprintf(input_fname, "%s/%s_%03d", path, basename, snapshot_number);
  sprintf(output_fname, "%s/%s_%03d", path, basenameout, snapshot_number);
  Ngas = load_snapshot(input_fname, files, output_fname, delx, dely, delz, boxsize);
