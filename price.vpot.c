...
//#define width_small 0.1 //which_sim==3
...
*/
	...
	sinkposz1 = zmax;
        ...
        xCOM = xCOM*cosmo_fac*exp_fac; yCOM = yCOM*cosmo_fac*exp_fac; zCOM = zCOM*cosmo_fac*exp_fac;
        ...
        //printf("line 313\n");       
         ...
         B0z = 0.0;
          ...
          for(i = 0; i < Ngas; i++)
             ...
             P[i].Pos[0] = (P[i].Pos[0] + 0.)*cosmo_fac*exp_fac;
             P[i].Pos[1] = (P[i].Pos[1] + 0.)*cosmo_fac*exp_fac;
             P[i].Pos[2] = (P[i].Pos[2] + 0.)*cosmo_fac*exp_fac;
             P[i].hsm = P[i].hsm*cosmo_fac*hfac_out*exp_fac; 

             A0x = ashift + B0x * (P[i].Pos[1]-yshift);
             A0y = ashift + B0y * (P[i].Pos[2]-zshift);
             A0z = ashift + B0z * (P[i].Pos[0]-xshift);              
...
/* 
               for(p=0; p<3; p++)
                  ...
                  for(k=0; k<plist1[n].NumNeighb; k++)
                    {
                    //nb[n-nmin].pos1[p][k] = nb[n-nmin].pos1[p][k] - plist1[n].pos[p];
                    nb[n-nmin].pos0[p][k] = nb[n-nmin].pos0[p][k] - plist0[n].pos[p];
                    }
                  }
                for(p=0; p<3; p++)
                  {
                  //plist1[n].pos[p] = plist1[n].pos[p] - plist1[n].pos[p];
                  plist0[n].pos[p] = plist0[n].pos[p] - plist0[n].pos[p];
                  }
*/
...
*/
                 ...
                 //A_exp = plist0[n].A_anal[0];
                 printf("j = %d, n = %d, n_neighb0 = %d, n_neighb1 = %d, dens0 = %lg, dens1 = %lg, dAdt[0] = %lg, dAdt[1] = %lg, dAdt[2] = %lg, fcor1 = %lg, A_anal0[0] = %lg, A_anal0[1] = %lg, A_anal0[2] = %lg hsm0 = %lg, hsm1 = %lg, vel_fac = %lg, det_J = %lg, dWdu0 = %lg, A_exp = %lg, kernel = %lg,  A_anal[0] = %lg, A_anal[1] = %lg, ERROR = %lg\n",
