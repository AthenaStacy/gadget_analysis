...
#define width_small 10.0  //which_sim == 0
//#define width_small 5.0  //Orion2 box distance in pc
//#define width_small 0.5  //which_sim == 4
#define ref_lev 32
...
int main(int argc, char **argv)
{
  ...
  double disx, disy, disz, time_fac, time_fac0, cosmo_fac, cosmo_fac0;
...
///////////////////////////////get ID of particles whose evolution was followed////////////////
  ...
  for(i = 0; i < Ngas; i++)
     ...
     if(disx < width_small/2.0 && disy < width_small/2.0 && disz < width_small/2.0)
        idnum++;
     }
...
///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  int snapwrite, nskip = 1;
  double nh_typ=0, b_typ=0, b_typ0, u_typ=0, num_typ=0, bfield, ufield, u_norm, bx_typ=0, by_typ=0, bz_typ=0, ax_typ=0, ay_typ=0, az_typ=0, a_anal=0, a_typ;
  double dwdx, dwdy, dwdz, ax, ay, az, rcheck;

  //for(j=42;j<=42;j=j+5){
  //for(j=2500;j<=2500;j=j+5){
  //for(j=5977;j<=5977;j=j+5){
  //for(j=8224;j<=8224;j=j+5){
  //for(j=7130;j<=7130;j=j+5){
  for(j=0;j<snapnum;j=j+nskip){
  //for(j=0;j<snapnum-1;j=j++){

  snapshot_number= snaparr[0]; 
  snapwrite = snaparr[j+1];
  ...
  sprintf(output_fname2, "bfield_cosmo");
  ...
  if(j == 0 && which_sim == 4) {time_fac = 0.025; time_fac0 = 0.025; cosmo_fac = time_fac/(hubble_param);}
  if(j > 0)  time_fac = time_fac0 + 0.015 * (j_doub/snapnum_doub);
  //if(which_sim == 4) time_fac = 1.0;

  //cosmo_fac0 = cosmo_fac; b_typ0 = b_typ;
...
if(j > 0)
{
    sprintf(input_fname2, "%s/%s_bfield_%04d", path2, basename2, snapwrite);
    if(which_sim == 1)
...
//////////////////////////////////////////////////////////////////////////////////
  ...
  nh_typ = b_typ = u_typ = num_typ = bx_typ = by_typ = bz_typ = ax_typ = ay_typ = az_typ = a_anal=0;
  ...
  //if(j==0)  cosmo_fac0 = cosmo_fac;

  b_anal = b0 * pow(cosmo_fac0/cosmo_fac,2);
  //b_anal = b_typ0 * pow(cosmo_fac0/cosmo_fac,2);
  a_anal = a0 * pow(cosmo_fac0/cosmo_fac,1); 

  printf("j= %d, time_fac = %lg, cosmo_fac = %lg, cosmo_fac0 = %lg, b_typ = %lg, b_typ0 = %lg\n", j, time_fac, cosmo_fac, cosmo_fac0, b_typ, b_typ0);
