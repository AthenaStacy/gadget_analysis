...
int write_snapshot(char *fname, int files, char *outname)
{
  ...
  int Idnew; 
  int nnew = 128;  //number of "child" particles to replace "parent" particle
  double *pos, massnew, hsmnew, x, y, z, dis; 
  double nnew_doub = 128;
  double randomx, randomy, randomz;
  double delx, dely, delz, nh, nhmax;
  double mmax = 5.e-10;
  double dmax = 10.0;  //size of 'cut-out' square in units of PARSECS
  ...
  for(i=0, pc=1; i<files; i++, pc=pc_new)
      ...
      for(k=0,pc_new=pc;k<6;k++)
	{
      ...
      if(ntot_withmasses>0)
      {
        ...
        //Find position of densest particle
        nhmax = 0;
        for(n=0;n<Ngas;n++)
             {
             nh = P[n].Rho;
      ...
      //calculate x, y, and z distances (disx, disy, and disz) of all particles from the densest particle
      //disx, disy, and disz are in units of PARSECS
      for(k=0,pc_new=pc;k<6;k++)
        {
          for(n=0;n<header1.npart[k];n++)
              {
          ...
          for(n=0;n<header1.npart[k];n++)
            {
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax)  
                NumPart_new++;
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax && k ==0 && P[pc_new].Mass < mmax)
                Ngas_new++;
              pc_new++;
            }
        }
   ...
   if(header_old.npart[0]>0)
	{
	  ...
	  for(n=0, pc_sph=pc; n<header_old.npart[0];n++)
	    {
              if(P[pc_sph].disx < dmax && P[pc_sph].disy < dmax && P[pc_sph].disz < dmax && P[pc_sph].Mass < mmax) 
                {
                if(n%10000 == 0)
                     printf("U = %lg\n",P[pc_sph].U);
		for(l=0;l<nnew;l++)
