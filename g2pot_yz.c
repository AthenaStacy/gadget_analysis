...
struct particle_data
  ...
  double  U, Temp, nh, Density, Hsml, pot;
...
//    int n_grid2[N_grid][N_grid];
    ...
    double entries = 0.0;
    double temp_pos = 0.0;
    //double min = 1.e11;       //bin_HR density range
    //double max = 2.e11;
    double min = 1.e10;
    double max = 1.e12;
    double abs_min = min;
    ...
    double center_i, center_j, W_x, weight, h, x2, x, pot_fac, pot_min=1.e30, pot_max = -1.e30;
    ...
    if(halo == 7)
       sprintf(dir, "/work/00863/minerva/bin_zoom7_wpot_nogas");
    if(halo == 8)
    ...
    if(halo == 10)
       sprintf(dir, "/work/00863/minerva/bin_zoom10_wpot");
    if(halo == 11)
       sprintf(dir, "/work/00863/minerva/bin_HR10_wpot");
    if(halo == 12)
       sprintf(dir, "/Volumes/Apps_and_Docs/astacy/SimData/binHRE/bin_HRE10");	 
    if(halo == 13)
    ...
    for(m = N_begin; m < N_end+1; m=m+1)
      {
        if(m < 10)
          {
            ...
            sprintf(buf, "%s_00%d.potyz", dir, m);
        ...
        else
          {
            ...
            sprintf(buf, "%s_%d.potyz", dir, m);

            printf("processing %d...\n", m);
          }

        width= 0.2*header.HubbleParam/(1.e3*header.time);  //0.2 pc
        slice= 0.02*header.HubbleParam/(1.e3*header.time);

        //width= 250.*4.85e-4*header.HubbleParam/(1.e3*header.time);  //24,000 AU
        //slice= 250.*4.85e-4*header.HubbleParam/(1.e3*header.time);       
        ...
        IDmass = 0.0;
        IDsink = 0;
        pot_fac = pow(UnitLength_in_cm,2) / pow(UnitTime_in_s,2) / pow(header.time/header.HubbleParam,1);


        for(n = 0; n < N_gas; n++)
          {
            P[n].nh = P[n].Density*unit_mass/pow(unit_length,3.0)*pow(header.HubbleParam,2.0)/pow(header.time,3.0)*X/m_H;
            P[n].pot = P[n].pot*pot_fac;
            if (n%100000 ==0) printf("%lg\n", P[n].pot);
            //if(fabs(P[n].pot) <= 1.e8)
              //printf("low potential! pot = %lg, nh = %lg\n", P[n].pot, P[n].nh);
            if (P[n].pot >= 0)
              P[n].pot = -1.e1;
          }
	...
	center_z=zCOM/ncount_doub;
        ...
        for(n = 0; n < N_gas; n++)
          {
          if(P[n].pot < pot_min && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[0]-center_x)) < slice/2.0)
            {
            pot_min = P[n].pot;
            }
          if(P[n].pot > pot_max && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[0]-center_x)) < slice/2.0)
            {
            pot_max = P[n].pot;
            }
          } 
...
/*      
        ...
        min = 0;
        max = pot_max - pot_min;
        abs_max = pot_max - pot_min;

printf("abs_min = %lg, abs_max = %lg\n", abs_min, abs_max);


        for(n = 0; n < N_gas; n++)
          {
            if(P[n].sink > -1 && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[0]-center_x)) < slice/2.0)
              {
...
x2 = ((P[n].Pos[1] - center_i) * (P[n].Pos[1] - center_i) + (P[n].Pos[2] - center_j) * (P[n].Pos[2] - center_j)) / h / h;
...
if(x2 <= 1.0)
  {
    ...
    n_grid1[i][j] += weight * W_x;
        ...
        grid1[0][0] = abs_min;
        grid1[N_grid-1][N_grid-1] = abs_max;
        ...
        for(j = 0; j < N_grid; j++)
          {
            ...
            grid3[0][j] = (min)+(2.0*j+1.0)*((max)-(min))/double(N_grid)/2.0;
          }
