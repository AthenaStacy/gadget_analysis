        ...
        if(j > 0)
         ...
         for(n=nmin; n<=nmax; n++)
          ...
          for(n=nmin; n<=nmax; n++)
             ...
             xmin = plist0[n].pos[0] - plist0[n].hsm; imin = (int) (xmin / header1.BoxSize * n_doub);
             xmax = plist0[n].pos[0] + plist0[n].hsm; imax = (int) (xmax / header1.BoxSize * n_doub);
             ymin = plist0[n].pos[1] - plist0[n].hsm; jmin = (int) (ymin / header1.BoxSize * n_doub); 
             ymax = plist0[n].pos[1] + plist0[n].hsm; jmax = (int) (ymax / header1.BoxSize * n_doub);
             ...
             for(i = 0; i < npart[i_dom][j_dom]; i++)
                {
                 ...
                 if(x0 + plist0[n].hsm > header1.BoxSize && plist0[n].pos[0] - plist0[n].hsm < 0)
                   x0_alt = x0 - header1.BoxSize;
                 if(y0 + plist0[n].hsm > header1.BoxSize && plist0[n].pos[1] - plist0[n].hsm < 0)
                   y0_alt = y0 - header1.BoxSize;
                 if(x0 - plist0[n].hsm < 0 && plist0[n].pos[0] + plist0[n].hsm  > header1.BoxSize)
                   x0_alt = x0 + header1.BoxSize;
                 if(y0 - plist0[n].hsm < 0 && plist0[n].pos[1] + plist0[n].hsm  > header1.BoxSize)
                   y0_alt = y0 + header1.BoxSize;
                 ...
                 if(x1 + plist1[n].hsm > header1.BoxSize && plist1[n].pos[0] - plist1[n].hsm < 0)
                   x_alt = x1 - header1.BoxSize;
                 if(y1 + plist1[n].hsm > header1.BoxSize && plist1[n].pos[1] - plist1[n].hsm < 0)
                   y_alt = y1 - header1.BoxSize;
                 if(x1 - plist1[n].hsm < 0 && plist1[n].pos[0] + plist1[n].hsm  > header1.BoxSize)
                   x_alt = x1+ header1.BoxSize;
                 if(y1 - plist1[n].hsm < 0 && plist1[n].pos[1] + plist1[n].hsm  > header1.BoxSize)
                   y_alt = y1 + header1.BoxSize;
             
                  //if we have a 2D sim
                  z0_alt = plist0[n].pos[2] + 0.001*plist1[n].hsm;
                  z_alt  = plist1[n].pos[2] + 0.001*plist1[n].hsm;
             
                 if((rad <= hfac*plist0[n].hsm || rad_alt <= hfac*plist0[n].hsm) && rad > hfac_in*plist0[n].hsm && plist1[n].NumNeighb < neighbnum)
                     {
                     dis_min = rad;
...
*/
      ...
      ///////////////////////////////////////////////////////////////////////////
                 ...
                 //if(j > 1)  for(p=0; p<3; p++) plist0[n].B_anal[p] = plist0[n].Bsmooth[p];
                ...
                for(p=0; p<3; p++)
                 ...
                 printf("n = %d, n_neighb = %d, n_neighb1 = %d, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, z0 = %lg, z1 = %lg, hsm0 = %lg, hsm1 = %lg, deform = %lg, det_jacob = %lg, detD_anal = %lg, ERROR = %lg, B_anal[0] = %lg, B_anal[1] = %lg, B_anal[2] = %lg\n",
                    n, plist1[n].NumNeighb, plist1[n].NumCalc, plist0[n].nh_anal, plist1[n].dens,  plist1[n].nh_anal, plist0[n].pos[2], plist1[n].pos[2], plist0[n].hsm, plist1[n].hsm, jacob_fin[0][0],
...
*/
  ...
  //For NON-cosmological runs
  if(which_sim < 4) {Time = 1.0; zred = 0;}
