...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  int    t,n,off,pc=0,pc_new,pc_sph=0;
  ...
  double ngrid2_x=1, ngrid2_y=1, ngrid2_z=1, ngrid_x=1, ngrid_y=1, ngrid_z=1;
...
//////////write new file!!!!!!!!!!!
      ...
      for(k=0,pc_new=pc;k<6;k++)
          ...
          for(n=0;n<header_old.npart[k];n++)
            {
              ...
              if(P[pc_new].disx < dmax && P[pc_new].disy < dmax && P[pc_new].disz < dmax && k ==0 && P[pc_new].Mass < mass_max)
                ...
                for(l=0;l<nnew2;l++)
                  {
                     num++;
                     frac_x = ngrid2_x/ngrid2_tot;  frac_y = ngrid2_y/ngrid2_tot;  frac_z = ngrid2_z/ngrid2_tot;
                     ...
                     x = header1.BoxSize*frac_x / 2. + header1.BoxSize/4.;   
                     y = header1.BoxSize*frac_y / 2. + header1.BoxSize/4.;  
                     z = header1.BoxSize*frac_z / 2. + header1.BoxSize/4.;
		...
		for(l=0;l<nnew;l++)
		  {
                     num++;
                     frac_x = ngrid_x/ngrid_tot;  frac_y = ngrid_y/ngrid_tot;  frac_z = ngrid_z/ngrid_tot;
                     ...
                     x = header1.BoxSize*frac_x; //fill each direction uniformly    
                     y = header1.BoxSize*frac_y;  
                     z = header1.BoxSize*frac_z;
      ...
      printf("positions finished, num = %lg, pc_new = %d\n", num, pc_new);
