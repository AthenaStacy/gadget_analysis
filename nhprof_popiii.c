...
int main(int argc, char **argv)
{
  ...
  int  i, k=0, j, jnum, n, type, snapshot_number, files, Ngas, random, ncount, ncounthalo1, arrnum=200;
  ...
  double h2_arr[arrnum], hd_arr[arrnum], hii_arr[arrnum], dii_arr[arrnum], heii_arr[arrnum];
  ...
  double vradx, vrady, vradz, vrotx, vroty, vrotz, vrad_arr[arrnum], vrot_arr[arrnum]; 
  double dens_arr[arrnum], c_s_arr[arrnum]; 
  double vradx_arr[arrnum], vrady_arr[arrnum], vradz_arr[arrnum]; 
  double vrotx_arr[arrnum], vroty_arr[arrnum], vrotz_arr[arrnum];
  ...
  //sprintf(path, "/global/scratch/minerva/cosmo_bfield");
  ...
  sprintf(output_fname, "%s_nhprof_%04d.dat",basename, snapshot_number);
  ...
  nthresh = 1.e4;	  	
	...
	rmin = 100.;
        ...
        for(n=0; n<arrnum; n++)
          ...
          temp_arr[n] = mbe_arr[n] = omega_arr[n] = 0;
          h2_arr[n] = hd_arr[n] = hii_arr[n] = dii_arr[n] = heii_arr[n] = 0.0; 
         ...
         for(i = 0; i < Ngas; i++)
		   ...
		   ncount++;  
                 ...
                 if(P[i].nh > 1.e-2)
                   for(n=0; n<arrnum; n++)
                    ...
                    if(P[i].nh > numdens_arr[n] && P[i].nh < numdens_arr[n+1] && P[i].sink < 0.5)
                      ...
                      hd_arr[n]  = hd_arr[n]  + P[i].HDI;
                      hii_arr[n] = hii_arr[n] + P[i].HII;
                      heii_arr[n] = heii_arr[n] + P[i].HeII;
                      dii_arr[n] = dii_arr[n] + P[i].DII;
             ...
             for(n=0; n<arrnum; n++)
                   ...
                   hd_arr[n] = hd_arr[n]/n_arr[n];
                   hii_arr[n] = hii_arr[n]/n_arr[n];
                   heii_arr[n] = heii_arr[n]/n_arr[n];
                   dii_arr[n] = dii_arr[n]/n_arr[n];
	           ...
	           turb_arr[n] = pow(velx_arr[n] - vradx_arr[n] - vrotx_arr[n],2) + pow(vely_arr[n] - vrady_arr[n] - vroty_arr[n],2) + pow(velz_arr[n] - vradz_arr[n] - vrotz_arr[n],2);
        		...
        		amom_arr[n] = m_enc_arr[0]*vrot_arr[0]*d_arr[0]*1.5e13*1.e5*2.e33;
                   ...
                   fprintf(outfile, "%15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g %15.11g\n", numdens_arr[n], h2_arr[n], hd_arr[n], dii_arr[n], heii_arr[n], hii_arr[n], m_enc_arr[n], dens_arr[n], surf_dens_arr[n], c_s_arr[n], temp_arr[n], mbe_arr[n], omega_arr[n]/m_enc_arr[n], vrad_arr[n], vrot_arr[n], turb_arr[n], amom_arr[n]);
                   }
             fclose(outfile);
...
/*	  MeanWeight= 4.0/(3*Xh+1+4*Xh*P[i].elec) * PROTONMASS;  */
	  ...
	  P[i].nh= P[i].Rho * HubbleParam * HubbleParam * pow(1.e0+zred,3.e0) * Xh / PROTONMASS;
