...
//    int n_grid2[N_grid][N_grid];
    ...
    for(m = N_begin; m < N_end+1; m=m+1)
      {
        if(m < 10)
        ...
        else
          {
            ...
            sprintf(buf, "%s_%d.pot", dir, m);

            printf("processing %d...\n", m);
          }

        width= 1.0*header.HubbleParam/(1.e3*header.time);  //1.0 pc
        slice= 1.0*header.HubbleParam/(1.e3*header.time);
	...
	center_z=zCOM/ncount_doub;
        ...
        for(n = 0; n < N_gas; n++)
          {
          if(P[n].pot < pot_min && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < slice/2.0)
            {
            pot_min = P[n].pot;
            }
          if(P[n].pot > pot_max && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < slice/2.0)
            {
            pot_max = P[n].pot;
            }
          } 
