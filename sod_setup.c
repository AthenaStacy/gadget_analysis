...
//////////write new file!!!!!!!!!!!
      ...
      num_right_tot = (1./9.) * double(NumPart_new);
      ...
      for(k=0,pc_new=pc;k<6;k++)
		...
		for(l=0;l<nnew;l++)
		  {
		      randomx =rand();
		      randomx = randomx/RAND_MAX*1.125;
		      randomy =rand();
		      ...
		      randomz = randomz/RAND_MAX;
                      ...
                      if(num_right <= num_right_tot)
                        ...
                        x = 0.5*header1.BoxSize*frac_xright + 0.5*header1.BoxSize;  //right side of shock tube                                                 
                        ...
                        {
                        num_left++;
                        frac_xleft = ngrid_xleft/ngrid_left_tot;
                        frac_yleft = ngrid_yleft/ngrid_left_tot;
                        frac_zleft = ngrid_zleft/ngrid_left_tot;

                        //if(ngrid_xleft >= ngrid_left_tot)
                        if(frac_xleft >= 1.)
                          {
                          ngrid_xleft = 0;
                          ngrid_yleft++;
                          }
                        //if(ngrid_yleft >= ngrid_left_tot)
                        if(frac_yleft >= 1.)
                          {
                          ngrid_yleft = 0;
                          ngrid_zleft++;
                        ...
                        x = 0.5*header1.BoxSize*frac_xleft;                        //left side of shock tube
                        y = header1.BoxSize*frac_yleft;
                        z = header1.BoxSize*frac_zleft;
...
/*		
	              if(randomx > 1.0)
                        ...
                        randomx = randomx/RAND_MAX;
       	                x = 0.5*header1.BoxSize*randomx + 0.5*header1.BoxSize;  //right side of shock tube
                        }
                      else        
                        {
                        randomx =rand();
                        randomx = randomx/RAND_MAX;
                        x = 0.5*header1.BoxSize*randomx;                        //left side of shock tube
                        }
		      //y = 0.1*header1.BoxSize*randomy + 0.5*header1.BoxSize - 0.05*header1.BoxSize;
		      //z = 0.1*header1.BoxSize*randomz + 0.5*header1.BoxSize - 0.05*header1.BoxSize;
			
                      y = header1.BoxSize*randomy;
                      z = header1.BoxSize*randomz;
*/
                      P[pc_new].Pos[0]=x;
                      P[pc_new].Pos[1]=y;
                      P[pc_new].Pos[2]=z;
