...
int main(int argc, char **argv)
{
  ...
  //snapshot_number= 120;                   /* number of snapshot */
  snapshot_number= 1;
  files=1;     

  sprintf(input_fname, "%s/%s_%03d", path, basename, snapshot_number);
  //sprintf(output_fname, "/work/00863/minerva/tgrid_g2shift_120");
  sprintf(output_fname, "/nobackupp1/astacy/tgrid_g2shift_001");
...
/*      printf("N_DM= %6d \n",NumPart-Ngas); */
 ...
 //     {
   ...
   //   if(ntot_withmasses>0)
   //   {
        printf("ntot_withmasses is bigger than 0!\n");
	SKIP;
        WSKIP;
   //   }
...
/*	       printf("h_smooth= %15.6g \n",P[pc_sph].hsm); */
	  ...
	  for(n=0, pc_sph=pc; n<header1.npart[0];n++)
	   ...
	   SKIP;
          ...
          for(n=0, pc_sph=pc; n<header1.npart[0];n++)
            {
              dum6[0]=(double)P[pc_sph].H2I;
              dum6[1]=(double)P[pc_sph].HII;
              dum6[2]=(double)P[pc_sph].DII;
              dum6[3]=(double)P[pc_sph].HDI;
              dum6[4]=(double)P[pc_sph].HeII;
              dum6[5]=(double)P[pc_sph].HeIII;
              fwrite(&dum6[0], sizeof(double), 1, outfile);
              fwrite(&dum6[1], sizeof(double), 1, outfile);
              fwrite(&dum6[2], sizeof(double), 1, outfile);
              fwrite(&dum6[3], sizeof(double), 1, outfile);
              fwrite(&dum6[4], sizeof(double), 1, outfile);
              fwrite(&dum6[5], sizeof(double), 1, outfile);
