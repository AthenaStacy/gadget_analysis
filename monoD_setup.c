...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  double ngrid_xleft=100, ngrid_yleft=100, ngrid_zleft=1, ngrid_xright=150, ngrid_yright=150, ngrid_z=1;
  double ngrid_tot = 0;
  double num_left=0, num=0, num_tot, num_tot_left;
  ...
  binit[0] = 0;  binit[1] = 0;  binit[2] = 1.e-19/sqrt(4*PI);
...
/////////////////////////////////////////////////////////////////////////////////////////
      ...
      NumPart_new = int(ngrid_xleft) * int(ngrid_yleft) * int(ngrid_z) 
                  + int(ngrid_xright) * int(ngrid_yright) * int(ngrid_z);
      Ngas_new = NumPart_new;
...
//////////write new file!!!!!!!!!!!
      ...
      num_tot_left = ngrid_xleft * ngrid_yleft * ngrid_z;
      ngrid_tot = pow(num_tot, 1./2.);

      int i, j, ngrid_x_int, ngrid_y_int, ngrid_z_int; 
      int ngrid_xleft_int, ngrid_yleft_int, ngrid_xright_int, ngrid_yright_int;
      ngrid_xright_int = int(ngrid_xright);
      ngrid_yright_int = int(ngrid_yright);
      ngrid_z_int = int(ngrid_z);
      ngrid_xleft_int = int(ngrid_xleft);
      ngrid_yleft_int = int(ngrid_yleft);


      WSKIP;
      pc_new = 0;
      for(i=0; i<ngrid_xleft_int; i++)
         for(j=0; j<ngrid_yleft_int; j++)
             for(k=0; k<ngrid_z_int; k++)
                 {
                 x = double(i+1) / ngrid_xleft / header1.BoxSize / 2. /*- header1.BoxSize/2.*/;
                 y = double(j+1) / ngrid_yleft / header1.BoxSize /*- header1.BoxSize/2.*/; 
                 z = (double(k + 0.5) - ngrid_z/2.) / ngrid_xleft  / header1.BoxSize;  
      ...
      for(i=0; i<ngrid_xright_int; i++)
         for(j=0; j<ngrid_yright_int; j++)
             for(k=0; k<ngrid_z_int; k++)
                 {
                 x = double(i+1) / (ngrid_xright) / header1.BoxSize / 2 + header1.BoxSize/2;
                 y = double(j+1) / (ngrid_yright) / header1.BoxSize;
                 z = (double(k + 0.5) - ngrid_z/2.) / ngrid_xleft  / header1.BoxSize;
      ...
      printf("positions finished, num = %lg, num_left = %lg, pc_new = %d\n", num, num_left, pc_new);
      ...
      for(k=0,pc_new=pc;k<6;k++)
	{
     ...
     rho_init = 1.0;
     volume = header1.BoxSize * header1.BoxSize * header1.BoxSize * 0.015625 * 0.5 *0.5;
     mass_tot = rho_init * volume;     
     massnew = mass_tot / num_tot_left; 
          ...
          for(n=0, pc_sph=pc; n<header1.npart[0];n++)
            {
              if(P[pc_sph].disx < dmax && P[pc_sph].disy < dmax && P[pc_sph].disz < dmax && P[pc_sph].Mass < mass_max)
                ...
                for(l=0;l<nnew;l++)
                  ...
                  x = P[pc_sph].Pos[0]-0.2;  
                  y = P[pc_sph].Pos[1]-0.2;   
                  r = sqrt(x*x + y*y);
                  if(r < r0) bwrite = 1.e-19 * (1/sqrt(4*PI)) * (pow(r/r0, 8) - 2*pow(r/r0, 4) + 1);
