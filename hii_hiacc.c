...
//    int n_grid2[N_grid][N_grid];
    ...
    double entries = 0.0;
    double temp_pos = 0.0;
    double min = 1.e-6;     //hires density range  
    double max = 1.e-1;
    double abs_min = min;
    double abs_max = max;
    ...
    sprintf(dir, "/work/00863/minerva/ifront_hiacc");
    sprintf(endtype, ".elec");
    if(rotate == 1) sprintf(endtype, ".elecxz");
    ...
    for(m = N_begin; m < N_end+1; m=m+1)
      {
        ...
        width= 500.*4.85e-6*header.HubbleParam/(1.e3*header.time);  //cut out central 500 AU
        //width= 0.1*header.HubbleParam/(1.e3*header.time);  //cut out central ? pc
        //width =  0.1625*header.HubbleParam/(1.e3*header.time);
...
*/
        ...
        center_x = xmax; center_y = ymax; center_z = zmax;
