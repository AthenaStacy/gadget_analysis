...
#define neighb_min 20  //which_sim==0
#define hfac_out 1.0
...
n_dom_x = (int) pow(tot_proc_sum, 0.5) * 4;
n_dom_y = n_dom_x;
...
for(i = 0; i < Ngas0; i++)
  ...
  //if(i%100000 == 0 && myrank == 0) printf("i = %d, i_dom = %d, j_dom = %d\n", i, i_dom, j_dom);
  if(i_dom > n_dom_x-1) i_dom = n_dom_x-1;
  if(j_dom > n_dom_y-1) j_dom = n_dom_y-1;
  npart[i_dom][j_dom]++;
  //if(i%100000 == 0 && myrank == 0) printf("i = %d, npart = %d\n", i, npart[i_dom][j_dom]);
  P0[i].x_dom = i_dom;
  P0[i].y_dom = j_dom; 
  }
