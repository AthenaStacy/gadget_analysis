...
#define arrnum 256 //should the total number of grid cells across corresponding Orion2 simulation
...
double calc_kernel_spline(int n, double x_part, double y_part, double z_part, double x, double y, double z, double hsm);
...
int main(int argc, char **argv)
{
  ...
  double gaccx[arrnum], gacc_xR[arrnum], gaccy[arrnum], gacc_yR[arrnum], gaccz[arrnum], gacc_zR[arrnum], DeltaX, dfac;
  ...
  DeltaX = width * pc_cm / double(arrnum);
  ...
  for(n=0;n<Ngas;n++)
     {
         ...
         //cen = 42;
         printf("cen1 = %lg, cen2 = %lg \n", d_arr[cen-cfac1], d_arr[cen+cfac2]); 
         ...
         for(n = 0; n < Ngas; n++)
             ...
             disx = (P[n].Pos[0]-sinkposx) * 1.e3*Time/(hubble) * 206264.806 - rmax/double(arrnum);
             disx_left  = disx - hsm_AU;
             disx_right = disx + hsm_AU;
             disy = (P[n].Pos[1]-sinkposy) * 1.e3*Time/(hubble) * 206264.806 - rmax/double(arrnum);
             disy_left  = disy - hsm_AU;
             disy_right = disy + hsm_AU;
             disz = (P[n].Pos[2]-sinkposz) * 1.e3*Time/(hubble) * 206264.806 - rmax/double(arrnum);
             ...
             for(k=1; k<arrnum; k++)
              ...
              for(k=1; k<arrnum; k++)
                {
                if(disx_left < d_arr[k] && disx_right > d_arr[k-1]  && disy_right > d_arr[cen - cfac1] && disz_right > d_arr[cen - cfac1] 
                                                           && disy_left < d_arr[cen + cfac2] && disz_left < d_arr[cen + cfac2])
                   {
                   //kernel = calc_kernel_spline(n, (d_arr[k]+d_arr[k-1])/2, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, disx, disy, disz, hsm_AU );
                   kernel = 1.;
                   nhx[k] = nhx[k] + P[n].nh * pow(kernel,2);
                   gpotx[k] = gpotx[k] + P[n].pot * pow(kernel,2);
                   gaccXx[k] = gaccXx[k] + P[n].Acc[0]*pow(kernel,2);
                   gaccXy[k] = gaccXy[k] + P[n].Acc[1]*pow(kernel,2);
                   gaccXz[k] = gaccXz[k] + P[n].Acc[2]*pow(kernel,2);
                   gaccx[k] = gaccx[k] + P[n].AccMag * pow(kernel,2);
                   numx[k] = numx[k] + pow(kernel,2);
                   }
                if(disy_left < d_arr[k] && disy_right > d_arr[k-1]  && disx_right > d_arr[cen - cfac1] && disz_right > d_arr[cen - cfac1] 
                                                           && disx_left < d_arr[cen + cfac2] && disz_left < d_arr[cen + cfac2])
                   {
                   //kernel = calc_kernel_spline(n, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, (d_arr[k]+d_arr[k-1])/2, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, disx, disy, disz, hsm_AU );
                   kernel = 1.;
                   nhy[k] = nhy[k] + P[n].nh * pow(kernel,2);
                   gpoty[k] = gpoty[k] + P[n].pot * pow(kernel,2);
                   gaccYx[k] = gaccYx[k] + P[n].Acc[0]*pow(kernel,2);
                   gaccYy[k] = gaccYy[k] + P[n].Acc[1]*pow(kernel,2);
                   gaccYz[k] = gaccYz[k] + P[n].Acc[2]*pow(kernel,2);
                   gaccy[k] = gaccy[k] + P[n].AccMag * pow(kernel,2);
                   numy[k] = numy[k] + pow(kernel,2);
                   }
                if(disz_left < d_arr[k] && disz_right > d_arr[k-1]  && disx_right > d_arr[cen - cfac1] && disy_right > d_arr[cen - cfac1] 
                                                           && disx_left < d_arr[cen + cfac2] && disy_left < d_arr[cen + cfac2])
                   {
                   //kernel = calc_kernel_spline(n, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, (d_arr[cen-cfac1] + d_arr[cen+cfac2])/2, (d_arr[k]+d_arr[k-1])/2, disx, disy, disz, hsm_AU );
                   kernel = 1.;
                   nhz[k] = nhz[k] + P[n].nh * pow(kernel,2);
                   gpotz[k] = gpotz[k] + P[n].pot * pow(kernel,2);
                   gaccZx[k] = gaccZx[k] + P[n].Acc[0]*pow(kernel,2);
                   gaccZy[k] = gaccZy[k] + P[n].Acc[1]*pow(kernel,2);
                   gaccZz[k] = gaccZz[k] + P[n].Acc[2]*pow(kernel,2);
                   gaccz[k] = gaccz[k] + P[n].AccMag * pow(kernel,2);
                   numz[k] = numz[k] + pow(kernel,2);
                   }
                 }
   ...
   /*
   for(n=1; n<arrnum-1; n++)
      {
      gaccx[n] = fabs(gpotx[n+1] - gpotx[n]) / DeltaX / 2.;
      gaccy[n] = fabs(gpoty[n+1] - gpoty[n]) / DeltaX / 2.;
      gaccz[n] = fabs(gpotz[n+1] - gpotz[n]) / DeltaX / 2.; 
      }
   */
