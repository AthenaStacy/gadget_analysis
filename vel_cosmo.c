...
int main(int argc, char **argv)
  {
    ...
    //int N_begin = 7010;
    //int N_end = 7010;
    //int N_begin = 679;   //which_sim==4
    //int N_end = 679;
    int N_begin = 445;  //which_sim == 6
    int N_end = 445;
...
//    int n_grid2[N_grid][N_grid];
    ...
    double entries = 0.0;
    double temp_pos = 0.0;
    //double min = 1.e4;   //velocity range (cm/s)
    double min = 1.e5;
    double max = 1.e6;
    double abs_min = min;
    double abs_max = max;
    ...
    if(which_sim == 4) 
      sprintf(dir, "/work/00863/minerva/binHR/bin_HR10_ideal");
    if(which_sim == 6)
      sprintf(dir, "/scratch/00863/minerva/bin_zoom10_ref4");
    sprintf(endtype, ".vel");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////// 
        ...
        double vel, vrad, vrot, vrotx, vroty, vrotz;
        double sinkposx, sinkposy, sinkposz;

        sinkposx = xCOM; sinkposy = yCOM; sinkposz = zCOM;
        ...
        for(n = 0; n < N_gas; n++)
          {
            if(P[n].sink > -1 && double(fabs(P[n].Pos[0]-center_x)) < width/2.0 && double(fabs(P[n].Pos[1]-center_y)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < width/2.0 && double(fabs(P[n].Pos[2]-center_z)) < slice/2.0)
             ...
             vel = pow(vel,0.5)*pow(Time, 0.5) * 1.e5;
             ...
             vrot = vrot/pow(((P[n].Pos[0]-sinkposx)*(P[n].Pos[0]-sinkposx) + (P[n].Pos[1]-sinkposy)*(P[n].Pos[1]-sinkposy) + (P[n].Pos[2]-sinkposz)*(P[n].Pos[2]-sinkposz)), 0.5);
             vrot = vrot*pow(Time, 0.5) * 1.e5;
...
if(x2 <= 1.0)
  {
    ...
    grid1[i][j] += weight * P[n].dummy * W_x;
...
if(x2 <= 1.0)
  {
    ...
    grid2[i][j] += weight * P[n].dummy * W_x;
