...
int main(int argc, char **argv)
{
  ...
  //sprintf(basename, "bin_zoom10_new_ref2");
  //sprintf(basename, "bin_zoom10_new_ref3");
  //sprintf(basename, "bin_zoom9_new_ref3");
  sprintf(basename, "bin_zoom9_new_ref3_wpot");
  //sprintf(basename, "bin_zoom10_new_ref3_wpot");
...
*/
  ...
  sprintf(basenameout, "bin_zoom9_new_cut_ref3_wpot");
...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double  boxsize)
{
  ...
  //double dmax = 100.e0;  //size of 'cut-out' square in pc
  double dmax = 10.e0;  //size of 'cut-out' square in pc
          ...
          for(n=0, pc_sph=pc; n<header_old.npart[0];n++)
            {
              if(P[pc_sph].disx < dmax && P[pc_sph].disy < dmax && P[pc_sph].disz < dmax && P[pc_sph].Mass < mass_max)
                {
                for(l=0;l<nnew;l++)
                  fwrite(&P[pc_sph].pot, sizeof(double), 1, outfile);
                }
              pc_sph++;
            }
          WSKIP;
         ...
         for(n=0, pc_sph=pc; n<header_old.npart[0];n++)
            {
              if(P[pc_sph].disx < dmax && P[pc_sph].disy < dmax && P[pc_sph].disz < dmax && P[pc_sph].Mass < mass_max)
                { 
                  for(l=0;l<nnew;l++) fwrite(&P[pc_sph].Acc[0], sizeof(double), 3, outfile);
                }
              pc_sph++;
            }
        WSKIP;
