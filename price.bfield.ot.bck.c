...
//// identify particles whose evolution shold be analytically followed
  ...
  if(which_sim == 0 || which_sim == 2) xmax = ymax = zmax = header1.BoxSize/2.0;
  if(which_sim == 0 || which_sim == 2) zmax = 0;
...
struct domain_data
  {
  double pos0[n_dom_x][n_dom_y][3], pos1[n_dom_x][n_dom_y][3];
  } *D;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(j > 0)
         ...
         for(n=nmin; n<=nmax; n++)
          ...
          double x0_alt, y0_alt, z0_alt, x_alt, y_alt, z_alt, rad_alt, kernel_alt;
          for(n=nmin; n<=nmax; n++)
             {
             i_dom = (int) (P0[i].Pos[0] / header1.BoxSize * n_doub);
             j_dom = (int) (P0[i].Pos[1] / header1.BoxSize * n_doub);
             for(i = 0; i < Ngas0; i++)
                {
                 ...
                 y0_alt = P0[i].Pos[1];
                 z0_alt = P0[i].Pos[2];
                 if(P0[i].Pos[0] + plist0[n].hsm > header1.BoxSize && plist0[n].pos[0] - plist0[n].hsm < 0)
                   x0_alt = P0[i].Pos[0] - header1.BoxSize;
                 if(P0[i].Pos[1] + plist0[n].hsm > header1.BoxSize && plist0[n].pos[1] - plist0[n].hsm < 0)
                   y0_alt = P0[i].Pos[1] - header1.BoxSize;
                 if(P0[i].Pos[0] - plist0[n].hsm < 0 && plist0[n].pos[0] + plist0[n].hsm  > header1.BoxSize)
                   x0_alt = P0[i].Pos[0] + header1.BoxSize;
                 if(P0[i].Pos[1] - plist0[n].hsm < 0 && plist0[n].pos[1] + plist0[n].hsm  > header1.BoxSize)
                   y0_alt = P0[i].Pos[1] + header1.BoxSize;

                 x_alt = P[i].Pos[0];
                 y_alt = P[i].Pos[1];
                 z_alt = P[i].Pos[2];
                 if(P[i].Pos[0] + plist1[n].hsm > header1.BoxSize && plist1[n].pos[0] - plist1[n].hsm < 0)
                   x_alt = P[i].Pos[0] - header1.BoxSize;
                 if(P[i].Pos[1] + plist1[n].hsm > header1.BoxSize && plist1[n].pos[1] - plist1[n].hsm < 0)
                   y_alt = P[i].Pos[1] - header1.BoxSize;
                 if(P[i].Pos[0] - plist1[n].hsm < 0 && plist1[n].pos[0] + plist1[n].hsm  > header1.BoxSize)
                   x_alt = P[i].Pos[0] + header1.BoxSize;
                 if(P[i].Pos[1] - plist1[n].hsm < 0 && plist1[n].pos[1] + plist1[n].hsm  > header1.BoxSize)
                   y_alt = P[i].Pos[1] + header1.BoxSize;


                 rad_alt = pow(((x_alt-plist0[n].pos[0])*(x_alt-plist0[n].pos[0]) + (y_alt-plist0[n].pos[1])*(y_alt-plist0[n].pos[1]) + (z_alt-plist0[n].pos[2])*(z_alt-plist0[n].pos[2])), 0.5);
...
////////////////////////////////////////
      ...
      ///////////////////////////////////////////////////////////////////////////
                ...
                for(p=0; p<3; p++)
                 ...
                 printf("n = %d, n_neighb = %d, n_neighb1 = %d, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, n_neighb1 = %d, hsm0 = %lg, hsm1 = %lg, deform = %lg, det_jacob = %lg, detD_anal = %lg, ERROR = %lg, B_anal[0] = %lg, B_anal[1] = %lg, B_anal[2] = %lg\n",
                    n, plist1[n].NumNeighb, plist1[n].NumCalc, plist0[n].nh_anal, plist1[n].dens,  plist1[n].rho_phys, plist1[n].NumCalc, plist0[n].hsm, plist1[n].hsm, jacob_fin[0][0],
...
*/
  ...
  //For NON-cosmological runs
  if(which_sim == 0 || which_sim == 2) {Time = 1.0; zred = 0;}
