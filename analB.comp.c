...
{
  char path[200], input_fname[200], input_fname2[200], plist_fname[200], output_fname[200], output_fname2[300]; 
  char basename[200], basename2[200], basenameout[200];
  int  ID, arrnum, i, k=0, ksort, j, n, p, q, r, s, type, snapshot_number, files, Ngas, Ngas0, random;
  double x,y,z,x1,y1,z1, delr, neighbnum_doub, nthresh = 1.e2, nthresh_cur, hfac = 0.75, hfac_in = 0.01, hfac_avg, mfac = 0.0, time_fac, cosmo_fac;
  ...
  double xfac, yfac, zfac, kfloat, nh_avg[snapnum], nh_avg2, num_tot, omega, omega0;
  double xtimesm, ytimesm, ztimesm, vxtimesm, vytimesm, vztimesm;
  double xCOM, yCOM, zCOM, vxCOM, vyCOM, vzCOM, jzcent;
  double B0=1.e-18, Bfield[snapnum], deform_avg[3][3]; 
  ...
  sprintf(path, "/work/00863/minerva/bin_map_smallt");
...
//// identify particles whose evolution shold be analytically followed
  ...
  for(i = 0; i < Ngas; i++)
     {
     disx = fabs((P[i].Pos[0]-xmax))*1.e3*Time/(hubble_param);
     disy = fabs((P[i].Pos[1]-ymax))*1.e3*Time/(hubble_param);
     disz = fabs((P[i].Pos[2]-zmax))*1.e3*Time/(hubble_param);
     if(disx < width_small && disy < width_small && disz < width_small)
        {
        arrnum++;
  ...
  if(restart == 1)
    ...
    sprintf(input_fname2, "%s/%s_bfield_%04d", path, basename, snapbegin);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(j == snapnum-2 ||(j%write_freq==0 && j > 1))
    {
  ...
  for(n=0; n<arrnum; n++)
      ...
      //plist1[n].vel_avg0 = plist1[n].vel_avg1 = 0;
      //plist1[n].vel_disp0 = plist1[n].vel_disp1 = 0;
      plist1[n].dens_alt = 0;
      ...
      for(p=0; p<3; p++)
       ...
       if(P[i].nh > nhmax /*&& P[i].sink < 0.5*/)
        ...
        nthresh_cur = nthresh_cur/ncount_doub/1.e3;  
             ...
             if(j == snapnum-2 || (j%write_freq==0 && j > 1))
                { 
                 if(P[i].marker > 0)
                   ...
                   PBfieldx[i] = 1.e-18 / (double) tot_proc_sum;
                   PBfieldy[i] = 1.e-18 / (double) tot_proc_sum;
                   PBfieldz[i] = 1.e-18 / (double) tot_proc_sum;
                   }
                 }
             ...
             if(P[i].nh > nthresh_cur) 
               for(n=nmin; n<=nmax; n++)
                      ...
                      if(P[i].Id == plist_id[n])
                         ...
                         plist1[n].dens_conv = P[i].nh/P[i].Rho;
...
*/
          ...
          if(j == snapnum-2 || (j%write_freq==0 && j > 1))
            ...
            if(myrank == 0)
              {
              sprintf(output_fname2, "%s_bfield_%04d",basename, snapshot_number);
              if(snapshot_number > 9999)
                sprintf(output_fname2, "%s_bfield_%05d",basename, snapshot_number);
