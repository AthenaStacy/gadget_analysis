...
//#define snapend 7133  //zoom10_ref3
...
int main(int argc, char **argv)
{
  ...
  if(which_sim == 2)
    ...
    sprintf(basename2, "bin_zoom9_new_cut_ref3");
    }
  if(which_sim == 3)
    {
    sprintf(path, "/scratch/00863/minerva"); 
             ...
             for(n=nmin; n<=nmax; n++)
                {
                 ...
                 //if(disx < hfac*plist1[n].hsm && disy < hfac*plist1[n].hsm && disz < hfac*plist1[n].hsm && rad > hfac_in*plist1[n].hsm && plist1[n].NumNeighb < neighbnum-1)
                     ...
                     nb[n].kernel1[k] = calc_kernel_spline(i, plist1[n].pos[0], plist1[n].pos[1], plist1[n].pos[2], P[i].Pos[0], P[i].Pos[1], P[i].Pos[2], plist1[n].hsm, cosmo_fac);
                     kernel1 = 1;
                     //kernel1 = nb[n].kernel1[k];
                     nb[n].pos1[0][k] = P[i].Pos[0] * pow(kernel1,1./3.);
                     nb[n].pos1[1][k] = P[i].Pos[1] * pow(kernel1,1./3.);
                     nb[n].pos1[2][k] = P[i].Pos[2] * pow(kernel1,1./3.);
             ...
             for(n=nmin; n<=nmax; n++)
               for(k=0; k<plist1[n].NumNeighb-1; k++)
                  ...
                  if(P0[i].Id == nb[n].id[k])
                     {
                     nb[n].kernel0[k] = calc_kernel_spline(i, plist0[n].pos[0], plist0[n].pos[1], plist0[n].pos[2], P0[i].Pos[0], P0[i].Pos[1], P0[i].Pos[2], plist0[n].hsm, cosmo_fac);                          
                     kernel0 = 1;
                     //kernel0 = nb[n].kernel0[k];
                     nb[n].pos0[0][k] = P0[i].Pos[0] * pow(kernel0,1./3.);
                     nb[n].pos0[1][k] = P0[i].Pos[1] * pow(kernel0,1./3.);
                     nb[n].pos0[2][k] = P0[i].Pos[2] * pow(kernel0,1./3.);
...
///////////////////////////////////////////////////////////
            ...
            for(n=nmin; n<=nmax; n++)
                ...
                for(k=0; k<plist1[n].NumNeighb-1; k++)
                  {
                  kernel_fac = (dis0[k] / plist0[n].hsm) /  (dis[k] / plist1[n].hsm);
                  //if(kernel_fac < 0.999 ||  kernel_fac > 1.001) printf("SKIP THIS PARTICLE!!\n"); 
                  //if(dis0[k] < hfac*plist0[n].hsm &&  kernel_fac > 0.999 &&  kernel_fac < 1.001)
                  if(dis0[k] < hfac*plist0[n].hsm && dis[k] < hfac*plist1[n].hsm)
                     {
                     for(p=0; p<3; p++)
                      { 
                       if(fabs(plist0[n].pos[p] - nb[n].pos0[p][k]) < hfac_avg*plist1[n].hsm)  
                         {
                         kernel_fac = nb[n].kernel0[k];
                         //kernel_fac = 1;
                ...
                for(p=0; p<3; p++)
                 ...
                 for(p=0; p<3; p++)
                   {
                   if(numHI1[p] <= 2 || numHI0[p] <= 2)
                     {
                     plist1[n].detD = 1;
                     deform_avg[p][p] = 0;
                     }
                   if((numHI1[p] <= 2 || numHI0[p] <= 2) && plist0[n].dens > 0 && plist1[n].dens > 0)
                 ...
                 if(n < 5) 
                 printf("n = %d, n_neighb = %d, dens0 = %lg, nh_anal0 = %lg, dens1 = %lg, nh_anal1 = %lg, dens_alt1 = %lg, hsm0 = %lg, hsm1 = %lg, deform = %lg, detD = %lg, detD_anal = %lg, detD_alt = %lg. B_anal[0] = %lg\n",
