...
//    int n_grid2[N_grid][N_grid];
    ...
    if(halo == 7)
	  sprintf(dir, "/Volumes/Apps_and_Docs/astacy/SimData/binHR/bin_HR7b");
	if(halo == 8)
	...
	if(halo == 10)
	  sprintf(dir, "/work/00863/minerva/bin_HR10");
        if(halo == 11)
          sprintf(dir, "/Volumes/Apps_and_Docs/astacy/SimData/binHRE/bin_HRE7");
        if(halo == 12)
    ...
    for(m = N_begin; m < N_end+1; m=m+1)
      {
        ...
        width= 0.8*header.HubbleParam/(1.e3*header.time);  //1 pc
        slice= (1./((double) N_grid)) * width;
